...
class BillingController extends Controller
{
    ...
    {
        $patient = $this->patientForUser($request->user());
        if (!$patient || $invoice->patient_id !== $patient->id) return response()->json(['message' => 'Forbidden'], 403);
    ...
    {
        $patient = $this->patientForUser($request->user());
        if (!$patient || $claim->patient_id !== $patient->id) return response()->json(['message' => 'Forbidden'], 403);
