...
export class MessagingWebSocketService {
  ...
  private handleTypingEvent(data: UserTypingEvent): void {
    ...
    if (data.is_typing) {
      ...
      const timeout = setTimeout(() => {
        const updatedUsers = (this.messagingStore.typingUsers[conversationId] || [])
          .filter(user => user.id !== data.user_id)
  ...
  private handleNewMessage(message: GerayeMessage): void {
    ...
    const conversation = this.messagingStore.conversations.find(conv => 
      conv.id === (message.group_id || this.getDirectConversationId(message))
    )
