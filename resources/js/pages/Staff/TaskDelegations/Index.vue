...
import { ArrowUpDown, Calendar, Tag, DollarSign, AlertCircle } from 'lucide-vue-next'
    ...
    <section class="surface-panel p-6 space-y-6">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="inline-flex rounded-md border overflow-hidden">
          <button @click="switchScope('assigned')" :class="['px-4 py-2 text-sm font-medium', scope === 'assigned' ? 'bg-cyan-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-50']">Assigned to Me</button>
          <button @click="switchScope('created')" :class="['px-4 py-2 text-sm font-medium', scope === 'created' ? 'bg-cyan-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-50']">Created by Me</button>
        </div>
      </div>
      ...
      <div class="flex flex-col md:flex-row gap-4">
        <div class="flex-1">
          <input
            ...
            class="w-full rounded-lg border border-gray-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-cyan-500 bg-white/50 backdrop-blur-sm"
          />
        </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div>
              ...
              <select 
                ...
                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500 bg-white/50 backdrop-blur-sm"
              >
        ...
        <table class="w-full text-left text-sm print-table">
          ...
          <tbody>
            <tr
              ...
              :key="t.id"
              class="border-t border-gray-100 hover:bg-cyan-50/50"
            >
              ...
              <td class="px-4 py-3">
                <select
                  ...
                  class="rounded-lg border border-gray-300 px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-cyan-500 bg-white/50 backdrop-blur-sm"
              ...
              <td class="px-4 py-3">
                <span v-if="t.assigned_to === ($page.props.auth.user.staff?.id || 0)" class="text-cyan-600 font-medium">
                  Me
                </span>
                <span v-else>
              ...
              <td class="px-4 py-3">
                <span v-if="t.task_category" class="px-2 py-1 text-xs rounded-full bg-cyan-100 text-cyan-800">
                  {{ t.task_category }}
                </span>
                <span v-if="t.is_billable" class="ml-1 px-2 py-1 text-xs rounded-full bg-green-100 text-green-800">
                  <DollarSign class="w-3 h-3 inline" />
                </span>
              </td>
              <td class="px-4 py-3">
                <select
