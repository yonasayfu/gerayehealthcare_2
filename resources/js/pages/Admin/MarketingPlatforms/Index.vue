...
import { Download, FileText, Edit3, Trash2, Printer, ArrowUpDown, Eye, Search, ToggleLeft, ToggleRight } from 'lucide-vue-next'
...
interface MarketingPlatformPagination {
...
const props = defineProps<{
  marketingPlatforms: MarketingPlatformPagination;
  filters: {
    search?: string;
    sort?: string;
    direction?: 'asc' | 'desc';
...
watch([search, sortField, sortDirection, perPage, isActive], debounce(() => {
  ...
  router.get(route('admin.marketing-platforms.index'), params, {
    preserveState: true,
    replace: true,
  })
}, 500))
...
function exportData(type: 'csv' | 'pdf') {
  alert(`Exporting ${type} is not yet implemented for Marketing Platforms.`);
}

function printCurrentView() {
  alert('Printing current view is not yet implemented for Marketing Platforms.');
}

const printAllPlatforms = () => {
    alert('Printing all platforms is not yet implemented.');
};
...
function toggleStatus(id: number) {
  router.put(route('admin.marketing-platforms.toggle-status', id), {}, {
    preserveScroll: true,
    onSuccess: () => {
      router.reload({ only: ['marketingPlatforms'] });
    },
  });
}
</script>
...
<template>
  <Head title="Marketing Platforms" />
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="space-y-6 p-6 print:p-0 print:space-y-0">
      ...
      <div class="rounded-lg bg-muted/40 p-4 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4 print:hidden">
        <div>
          <h1 class="text-xl font-semibold text-gray-800 dark:text-white">Marketing Platforms</h1>
          <p class="text-sm text-muted-foreground">Manage all marketing platforms here.</p>
        </div>
        <div class="flex flex-wrap gap-2">
          <Link :href="route('admin.marketing-platforms.create')" class="inline-flex items-center gap-2 bg-cyan-600 hover:bg-cyan-700 text-white text-sm px-4 py-2 rounded-md transition">
            + Add Platform
          </Link>
          ...
          <button @click="printAllPlatforms" class="inline-flex items-center gap-1 text-sm px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-md dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-200">
        ...
        <table class="w-full text-left text-sm text-gray-800 dark:text-gray-200 print-table">
          <thead class="bg-gray-100 dark:bg-gray-800 text-xs uppercase text-muted-foreground print-table-header">
            <tr>
              ...
              <th class="px-6 py-3 cursor-pointer" @click="toggleSort('api_endpoint')">
                API Endpoint <ArrowUpDown class="inline w-4 h-4 ml-1 print:hidden" />
              </th>
          ...
          <tbody>
            <tr v-for="platform in marketingPlatforms.data" :key="platform.id" class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 print-table-row">
              <td class="px-6 py-4">{{ platform.name }}</td>
              <td class="px-6 py-4">{{ platform.api_endpoint ?? '-' }}</td>
              <td class="px-6 py-4">
                <span :class="{
                  'bg-green-100 text-green-800': platform.is_active,
                  'bg-red-100 text-red-800': !platform.is_active,
              ...
              <td class="px-6 py-4 text-right print:hidden">
                <div class="inline-flex items-center justify-end space-x-2">
                  <button @click="toggleStatus(platform.id)" :title="platform.is_active ? 'Deactivate' : 'Activate'"
                    :class="{
                      'text-green-600 hover:text-green-800 hover:bg-green-100 dark:hover:bg-green-900': platform.is_active,
                      'text-red-600 hover:text-red-800 hover:bg-red-100 dark:hover:bg-red-900': !platform.is_active,
                    }"
                    class="inline-flex items-center justify-center w-8 h-8 rounded-full"
                  >
                    <component :is="platform.is_active ? ToggleRight : ToggleLeft" class="w-4 h-4" />
                  </button>
                  <Link
                    :href="route('admin.marketing-platforms.show', platform.id)"
                  ...
                  <Link
                    :href="route('admin.marketing-platforms.edit', platform.id)"
                  ...
                  <button @click="destroy(platform.id)" class="text-red-600 hover:text-red-800 inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-red-100 dark:hover:bg-red-900" title="Delete">
                    <Trash2 class="w-4 h-4" />
                  </button>
                </div>
              </td>
            </tr>
            <tr v-if="marketingPlatforms.data.length === 0">
              <td colspan="4" class="text-center px-6 py-4 text-gray-400">No marketing platforms found.</td>
            </tr>
          </tbody>
        </table>
      </div>
...
@media print {
  ...
  .print-table th:nth-child(1), .print-table td:nth-child(1) { width: 30%; } /* Name */
  .print-table th:nth-child(2), .print-table td:nth-child(2) { width: 40%; } /* API Endpoint */
  .print-table th:nth-child(3), .print-table td:nth-child(3) { width: 15%; } /* Active */
