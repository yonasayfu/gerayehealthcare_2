...
import type { MarketingLead } from '@/types'; // Import MarketingLead type

const props = defineProps<{
  leads: MarketingLead[]; // Use MarketingLead[] for type safety
}>()
...
<template>
  <Head title="All Marketing Leads Print View" />
      ...
      <table class="w-full text-left text-sm text-gray-800">
        <thead class="bg-gray-100 text-xs uppercase text-muted-foreground print-table-header">
          <tr>
            ...
            <th class="px-6 py-3">Lead Code</th>
            <th class="px-6 py-3">Email</th>
            <th class="px-6 py-3">Phone</th>
            <th class="px-6 py-3">Status</th>
            <th class="px-6 py-3">Source Campaign</th>
            <th class="px-6 py-3">Assigned Staff</th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="leads.length === 0">
            <td colspan="7" class="text-center px-6 py-4 text-gray-400">No marketing leads found.</td>
          </tr>
          <tr v-for="lead in leads" :key="lead.id" class="border-b print-table-row">
