...
const props = defineProps<{
  partnerCommission: {
    ...
    created_at: string
    updated_at: string
    agreement?: { agreement_title: string } | null;
    referral?: { referral_date: string } | null;
...
const breadcrumbs: BreadcrumbItemType[] = [
  ...
  { title: `Commission #${props.partnerCommission.id}`, href: route('admin.partner-commissions.show', props.partnerCommission.id) },
]
    ...
    <div class="bg-white border border-4 rounded-lg shadow relative m-10">
        ...
        <div class="flex items-center justify-between p-5 border-b rounded-t">
            <h3 class="text-xl font-semibold">
                Partner Commission Details: #{{ partnerCommission.id }}
            </h3>
            <div class="flex items-center gap-3">
              <Link :href="route('admin.partner-commissions.index')" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 inline-flex items-center print:hidden" title="Close">
        ...
        <div class="p-6 space-y-6">
            <div class="bg-white dark:bg-gray-900 shadow rounded-lg p-8 space-y-8 print:shadow-none print:rounded-none print:p-0 print:m-0 print:w-auto print:h-auto print:flex-shrink-0">
                ...
                <div class="hidden print:block text-center mb-4 print:mb-2 print-header-content">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 print-document-title">Partner Commission Record Document</p>
                    <hr class="my-3 border-gray-300 print:my-2">
                </div>

                <div class="border-b pb-4 mb-4 print:pb-2 print:mb-2">
                  <h2 class="text-lg font-semibold text-gray-800 dark:text-white mb-4 print:mb-2">Commission Information</h2>
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-6 print:gap-y-2 print:gap-x-4">
                    <div>
                      ...
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerCommission.agreement?.agreement_title ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Referral Date:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerCommission.referral?.referral_date ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Invoice #:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerCommission.invoice?.invoice_number ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Amount:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerCommission.commission_amount }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Calculation Date:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerCommission.calculation_date }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Payout Date:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerCommission.payout_date ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Status:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerCommission.status }}</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-6 print:gap-y-2 print:gap-x-4">
                    <div>
                      ...
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerCommission.created_at ? format(new Date(partnerCommission.created_at), 'PPP p') : '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Last Updated:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerCommission.updated_at ? format(new Date(partnerCommission.updated_at), 'PPP p') : '-' }}</p>
                    </div>
                  </div>
                </div>
        ...
        <div class="p-6 border-t border-gray-200 rounded-b">
            <div class="flex flex-wrap gap-2">
              <Link :href="route('admin.partner-commissions.index')" class="btn btn-outline print:hidden">
                Back to List
              </Link>
              <button @click="printPage" class="btn btn-dark print:hidden">
                <Printer class="h-4 w-4" /> Print Current
              </button>
              <Link :href="route('admin.partner-commissions.edit', props.partnerCommission.id)"
                class="btn btn-primary print:hidden">
                <Edit3 class="w-4 h-4" /> Edit Commission
              </Link>
            </div>
        </div>
