...
import AppLayout from '@/layouts/AppLayout.vue' // Correct layout
import { Download, FileText, Edit3, Trash2, Printer, ArrowUpDown, Plus } from 'lucide-vue-next'
...
// Reactive refs for filters, matching the staff component
const search = ref(props.filters.search || '')
const sortField = ref(props.filters.sort_by || 'scheduled_at')
const sortDirection = ref(props.filters.sort_direction || 'desc')
const perPage = ref(props.filters.per_page || 10)

// Watcher to refetch data when filters change
        ...
        <table class="w-full text-left text-sm text-gray-800 dark:text-gray-200">
          <thead class="bg-gray-100 dark:bg-gray-800 text-xs uppercase text-muted-foreground">
            <tr>
              ...
              <th class="px-6 py-3 cursor-pointer" @click="toggleSort('scheduled_at')">
                Scheduled At <ArrowUpDown class="inline w-4 h-4 ml-1" />
              </th>
          ...
          <tbody>
            <tr v-for="visit in visitServices.data" :key="visit.id" class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
              ...
              <td class="px-6 py-4 text-right">
                <div class="inline-flex items-center justify-end space-x-2">
                  <Link :href="route('admin.visit-services.edit', visit.id)" class="inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-blue-100 dark:hover:bg-blue-900 text-blue-600" title="Edit">
                    <Edit3 class="w-4 h-4" />
                  </Link>
                  <button @click="destroy(visit.id)" class="text-red-600 hover:text-red-800 inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-red-100 dark:hover:bg-red-900" title="Cancel Visit">
            ...
            <tr v-if="visitServices.data.length === 0">
              <td colspan="5" class="text-center px-6 py-4 text-gray-400">No visits found.</td>
            </tr>
          </tbody>
        </table>
      </div>

      <div v-if="visitServices.links.length > 3" class="flex justify-end">
        <div class="flex items-center space-x-1">
          <Link
            v-for="(link, i) in visitServices.links"
            :key="i"
            :href="link.url || ''"
            v-html="link.label"
            :class="[
              'px-3 py-1 rounded-md text-sm',
              link.active ? 'bg-indigo-600 text-white' : 'hover:bg-gray-200 dark:hover:bg-gray-700',
