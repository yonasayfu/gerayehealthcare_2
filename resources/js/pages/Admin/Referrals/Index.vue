      ...
      <div class="flex flex-col md:flex-row justify-between items-center gap-4 print:hidden">
        <div class="relative w-full md:w-1/3">
          <input
            type="text"
            v-model="search"
            placeholder="Search by status, referral date..."
            ...
            <tr v-for="(referral, i) in referrals.data" :key="referral.id" class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 print-table-row">
              ...
              <td class="px-6 py-4 text-right print:hidden">
                <div class="inline-flex items-center justify-end space-x-2">
                    <Link :href="route('admin.referrals.show', referral.id)" class="btn-icon text-indigo-600" title="View Details">
  <Eye class="w-4 h-4" />
</Link>

                  <Link :href="route('admin.referrals.edit', referral.id)" class="btn-icon text-blue-600" title="Edit">
                    <Edit3 class="w-4 h-4" />
                  </Link>
                  <button @click="destroy(referral.id)" class="btn-icon text-red-600 hover:text-red-800" title="Delete">
