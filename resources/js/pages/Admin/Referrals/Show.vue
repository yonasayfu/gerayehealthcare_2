...
const breadcrumbs: BreadcrumbItemType[] = [
  ...
  { title: `Referral for ${props.referral.patient?.full_name ?? 'N/A'}`, href: route('admin.referrals.show', props.referral.id) },
]
...
<template>
  <Head :title="`Referral: ${props.referral.patient?.full_name ?? 'N/A'}`" />
    ...
    <div class="bg-white border border-4 rounded-lg shadow relative m-10">
        ...
        <div class="flex items-center justify-between p-5 border-b rounded-t">
            <h3 class="text-xl font-semibold">
                Referral Details: {{ referral.patient?.full_name ?? 'N/A' }}
            </h3>
            <div class="flex items-center gap-3">
              <Link :href="route('admin.referrals.index')" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 inline-flex items-center print:hidden" title="Close">
        ...
        <div class="p-6 space-y-6">
            <div class="bg-white dark:bg-gray-900 shadow rounded-lg p-8 space-y-8 print:shadow-none print:rounded-none print:p-0 print:m-0 print:w-auto print:h-auto print:flex-shrink-0">
                ...
                <div class="hidden print:block text-center mb-4 print:mb-2 print-header-content">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 print-document-title">Referral Record Document</p>
                    <hr class="my-3 border-gray-300 print:my-2">
                </div>

                <div class="border-b pb-4 mb-4 print:pb-2 print:mb-2">
                  <h2 class="text-lg font-semibold text-gray-800 dark:text-white mb-4 print:mb-2">Referral Information</h2>
        ...
        <div class="p-6 border-t border-gray-200 rounded-b">
            <div class="flex flex-wrap gap-2">
              <Link :href="route('admin.referrals.index')" class="btn btn-outline print:hidden">
                Back to List
              </Link>
              <button @click="printPage" class="btn btn-dark print:hidden">
                <Printer class="h-4 w-4" /> Print Current
              </button>
              <Link :href="route('admin.referrals.edit', props.referral.id)"
                class="btn btn-primary print:hidden">
                <Edit3 class="w-4 h-4" /> Edit Referral
              </Link>
            </div>
        </div>
