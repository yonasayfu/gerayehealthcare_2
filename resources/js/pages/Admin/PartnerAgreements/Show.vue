...
const props = defineProps<{
  partnerAgreement: {
    ...
    signed_by?: { first_name: string; last_name: string } | null;
  };
}>()
...
const breadcrumbs: BreadcrumbItemType[] = [
  ...
  { title: props.partnerAgreement.agreement_title, href: route('admin.partner-agreements.show', props.partnerAgreement.id) },
]
...
<template>
  <Head :title="`Partner Agreement: ${props.partnerAgreement.agreement_title}`" />
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
      ...
      <div class="liquidGlass-wrapper print:hidden w-full rounded-t-lg">
        ...
        <div class="liquidGlass-content flex items-center justify-between p-6">
          <div class="print:hidden">
            <h3 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Partner Agreement Details</h3>
            <p class="text-sm text-gray-600 dark:text-gray-300">Partner Agreement: {{ item.name || item.title || item.id }}</p>
          </div>
        ...
        <div class="p-6 space-y-6">
            <div class="bg-white dark:bg-gray-900 shadow rounded-lg p-8 space-y-8 print:shadow-none print:rounded-none print:p-0 print:m-0 print:w-auto print:h-auto print:flex-shrink-0">
                ...
                <div class="hidden print:block text-center mb-4 print:mb-2 print-header-content">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 print-document-title">Partner Agreement Record Document</p>
                    <hr class="my-3 border-gray-300 print:my-2">
                </div>

                <div class="border-b pb-4 mb-4 print:pb-2 print:mb-2">
                  <h2 class="text-lg font-semibold text-gray-800 dark:text-white mb-4 print:mb-2">Agreement Information</h2>
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-6 print:gap-y-2 print:gap-x-4">
                    <div>
                      ...
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerAgreement.partner?.name ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Title:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerAgreement.agreement_title }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Type:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerAgreement.agreement_type }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Status:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerAgreement.status }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Start Date:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerAgreement.start_date }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">End Date:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerAgreement.end_date ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Priority Service Level:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerAgreement.priority_service_level ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Commission Type:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerAgreement.commission_type ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Commission Rate:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerAgreement.commission_rate ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Terms Document Path:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerAgreement.terms_document_path ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Signed By Staff:</p>
                      <p class="font-medium text-gray-900 dark:text-white">
                        {{ partnerAgreement.signed_by ? `${partnerAgreement.signed_by.first_name} ${partnerAgreement.signed_by.last_name}` : '-' }}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-6 print:gap-y-2 print:gap-x-4">
                    <div>
                      ...
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerAgreement.created_at ? format(new Date(partnerAgreement.created_at), 'PPP p') : '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Last Updated:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ partnerAgreement.updated_at ? format(new Date(partnerAgreement.updated_at), 'PPP p') : '-' }}</p>
                    </div>
                  </div>
                </div>
      ...
      <div class="p-6 border-t border-gray-200 dark:border-gray-700 rounded-b print:hidden">
        <div class="flex justify-end gap-2">
          <Link :href="route('admin.partner-agreements.index')" class="btn-glass btn-glass-sm">Back to List</Link>
          <button @click="printSinglePartnerAgreement" class="btn-glass btn-glass-sm">Print Current</button>
          <Link :href="route('admin.partner-agreements.edit', item.id)" class="btn-glass btn-glass-sm">Edit</Link>
        </div>
      </div>
