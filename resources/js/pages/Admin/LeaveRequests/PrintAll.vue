...
const requests = computed(() => {
  return Array.isArray(props.leaveRequests) ? props.leaveRequests : [props.leaveRequests];
});
...
<template>
  <Head title="Print All Leave Requests" />
  ...
  <div class="print-document">
    ...
    <div v-else-if="requests.length === 0" class="no-data-message">
      <p>No leave requests found to print.</p>
    </div>
      ...
      <table class="print-table">
        ...
        <tbody>
          <tr v-for="request in requests" :key="request.id">
            <td>{{ getStaffFullName(request.staff) }}</td>
