...
watch([search, sortField, sortDirection, perPage], debounce(() => {
    ...
    router.get(route('admin.event-recommendations.index'), params, {
        preserveState: true,
        replace: true,
    });
}, 500));
            ...
            <div class="overflow-x-auto bg-white dark:bg-gray-900 shadow rounded-lg print:shadow-none print:rounded-none print:bg-transparent">
                <table class="w-full text-left text-sm text-gray-800 dark:text-gray-200 print-table">
                    <thead class="bg-gray-100 dark:bg-gray-800 text-xs uppercase text-muted-foreground print-table-header">
                        <tr>
                            <th class="px-6 py-3 cursor-pointer" @click="toggleSort('patient_name')">
                                Patient Name <ArrowUpDown class="inline w-4 h-4 ml-1 print:hidden" />
                            </th>
                  ...
                  >
                    <Trash2 class="w-4 h-4" />
                        ...
                        <tr v-if="recommendations.data.length === 0">
                            <td colspan="4" class="text-center px-6 py-4 text-gray-400">No event recommendations found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>

            <Pagination v-if="recommendations.data.length > 0" :links="recommendations.links" class="mt-6 flex justify-center print:hidden" />
