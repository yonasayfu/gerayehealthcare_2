...
<template>
  <Head :title="`Landing Page: ${landingPage.page_title}`" />
    ...
    <div class="bg-white border border-4 rounded-lg shadow relative m-10 print:border-0 print:shadow-none print:m-0">
        ...
        <div class="flex items-start justify-between p-5 border-b rounded-t print:hidden">
            <h3 class="text-xl font-semibold">
                Landing Page Details: {{ landingPage.page_title }}
            </h3>
            <Link :href="route('admin.landing-pages.index')" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
        ...
        <div class="p-6 space-y-6">
            <div class="bg-white dark:bg-gray-900 shadow rounded-lg p-8 space-y-8 print:shadow-none print:rounded-none print:p-0 print:m-0 print:w-auto print:h-auto print:flex-shrink-0">
                ...
                <div class="hidden print:block text-center mb-4 print:mb-2 print-header-content">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 print-document-title">Landing Page Document</p>
                    <hr class="my-3 border-gray-300 print:my-2">
                </div>

                <div class="border-b pb-4 mb-4 print:pb-2 print:mb-2">
                  <h2 class="text-lg font-semibold text-gray-800 dark:text-white mb-4 print:mb-2">Page Information</h2>
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-6 print:gap-y-2 print:gap-x-4">
                    <div>
                      ...
                      <p class="font-medium text-gray-900 dark:text-white">{{ landingPage.page_title }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Page Code:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ landingPage.page_code ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Page URL:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ landingPage.page_url ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Language:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ landingPage.language ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Template Used:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ landingPage.template_used ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Is Active:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ landingPage.is_active ? 'Yes' : 'No' }}</p>
                    </div>
                  </div>
                </div>

                <div class="border-b pb-4 mb-4 print:pb-2 print:mb-2">
                  <h2 class="text-lg font-semibold text-gray-800 dark:text-white mb-4 print:mb-2">Campaign & Conversion</h2>
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-6 print:gap-y-2 print:gap-x-4">
                    <div>
                      <p class="text-sm text-muted-foreground">Campaign:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ landingPage.campaign?.campaign_name ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Conversion Goal:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ landingPage.conversion_goal ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Views:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ landingPage.views ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Submissions:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ landingPage.submissions ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Conversion Rate:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ landingPage.conversion_rate ?? '-' }}</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div class="grid grid-cols-1 gap-y-3 gap-x-6 print:gap-y-2 print:gap-x-4">
                    <div class="col-span-full">
                      ...
                      <pre class="font-medium text-gray-900 dark:text-white bg-gray-100 dark:bg-gray-800 p-4 rounded-md overflow-auto">{{ formatJson(landingPage.form_fields) }}</pre>
                    </div>
                    <div class="col-span-full">
                      <p class="text-sm text-muted-foreground">Notes:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ landingPage.notes ?? '-' }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Created At:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ format(new Date(landingPage.created_at), 'PPP p') }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-muted-foreground">Updated At:</p>
                      <p class="font-medium text-gray-900 dark:text-white">{{ format(new Date(landingPage.updated_at), 'PPP p') }}</p>
                    </div>
                  </div>
                </div>
        ...
        <div class="p-6 border-t border-gray-200 rounded-b print:hidden">
            <div class="flex flex-wrap gap-2">
              ...
              <Link
                :href="route('admin.landing-pages.edit', landingPage.id)"
                class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
              >
                Edit Page
              </Link>
              <button @click="destroy(landingPage.id)" class="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white text-sm px-4 py-2 rounded-md transition">
                <Trash2 class="w-4 h-4" /> Delete Page
              </button>
            </div>
        </div>
