...
function exportCsv() {
  window.open(route('admin.partners.export', { type: 'csv' }), '_blank');
}
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="space-y-6 p-6 print:p-0 print:space-y-0">
      ...
      <div class="rounded-lg bg-muted/40 p-4 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4 print:hidden">
        <div>
          <h1 class="text-xl font-semibold text-gray-800 dark:text-white">Partners</h1>
          <p class="text-sm text-muted-foreground">Manage all partner records here.</p>
        </div>
        <div class="flex flex-wrap gap-2">
          <Link :href="route('admin.partners.create')" class="btn btn-primary">
            + Add Partner
          </Link>
          ...
          <button @click="printAllPartners" class="btn btn-info">
            ...
            <tr v-for="(partner, i) in partners.data" :key="partner.id" class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 print-table-row">
              ...
              <td class="px-6 py-4 text-right print:hidden">
                <div class="inline-flex items-center justify-end space-x-2">
                    <Link :href="route('admin.partners.show', partner.id)" class="btn-icon text-indigo-600" title="View Details">
  <Eye class="w-4 h-4" />
</Link>

                  <Link :href="route('admin.partners.edit', partner.id)" class="btn-icon text-blue-600" title="Edit">
                    <Edit3 class="w-4 h-4" />
                  </Link>
                  <button @click="destroy(partner.id)" class="btn-icon text-red-600 hover:text-red-800" title="Delete">
