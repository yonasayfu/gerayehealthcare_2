...
<template>
    <div class="print-container">
        <div class="print-header-content">
            ...
            <p class="text-gray-600 dark:text-gray-400 print-document-title">Campaign Contents List (All)</p>
        ...
        <table class="print-table">
            <thead>
                <tr>
                    ...
                    <th>Type</th>
                    <th>Status</th>
                    <th>Scheduled Post Date</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="content in campaignContents" :key="content.id">
                    <td>{{ content.title }}</td>
                    <td>{{ content.campaign?.campaign_name ?? '-' }}</td>
                    <td>{{ content.platform?.name ?? '-' }}</td>
                    <td>{{ content.content_type ?? '-' }}</td>
                    <td>{{ content.status ?? '-' }}</td>
                    <td>{{ content.scheduled_post_date ? format(new Date(content.scheduled_post_date), 'PPP p') : '-' }}</td>
                </tr>
            </tbody>
        </table>
