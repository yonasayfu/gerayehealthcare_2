...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: 'Reports', href: '/dashboard/reports/marketing-roi' },
  { title: 'Marketing ROI', href: '#' },
]
...
// Aggregated KPIs
const totalSpend = computed(() => rows.value.reduce((s, r) => s + (Number(r.spend) || 0), 0))
const totalRevenue = computed(() => rows.value.reduce((s, r) => s + (Number(r.revenue_generated) || 0), 0))
const roiPercent = computed(() => {
  const spend = totalSpend.value
  if (spend <= 0) return 0
  return ((totalRevenue.value - spend) / spend) * 100
})
...
async function fetchData() {
  ...
  try {
    ...
    const res = await fetch(`/dashboard/reports/marketing-roi/data${qs ? `?${qs}` : ''}`)
...
function exportCsv() {
  const qs = buildQuery()
  qs.set('type', 'csv')
  window.open(`/dashboard/reports/marketing-roi/export?${qs.toString()}`, '_blank')
}
...
<template>
  <Head title="Marketing ROI Report" />
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="space-y-6 p-4 md:p-6">
      <div>
        <h1 class="text-xl font-semibold">Marketing ROI</h1>
        <p class="text-sm text-muted-foreground">Quarterly / Half-Year / Yearly ROI and attribution performance.</p>
      </div>
      ...
      <div class="rounded-xl border border-border bg-white p-4 dark:border-sidebar-border dark:bg-background">
        <div class="grid gap-3 md:grid-cols-5">
          ...
          <div>
            ...
            <input v-model="platform" type="text" placeholder="e.g. Google" class="w-full rounded-md border p-2 text-sm" />
          </div>
      ...
      <div class="grid gap-4 md:grid-cols-3">
        ...
        <template v-else>
          <KpiCard title="Total Spend" :value="totalSpend.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})" color="orange" />
          <KpiCard title="Attributed Revenue" :value="totalRevenue.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})" color="blue" />
          <KpiCard title="ROI %" :value="roiPercent.toFixed(2) + '%'" color="green" />
        </template>
      </div>
      ...
      <div class="rounded-xl border bg-white p-4 dark:border-sidebar-border dark:bg-background">
        ...
        <div class="text-sm text-muted-foreground">Campaign Attribution Table Placeholder</div>
      </div>
    </div>
  </AppLayout>
</template>
