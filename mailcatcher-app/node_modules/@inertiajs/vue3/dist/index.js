...
var Form = (0, import_vue5.defineComponent)({
  ...
  setup(props, { slots, attrs, expose }) {
    ...
    const onFormUpdate = (event) => {
      isDirty.value = event.type === "reset" ? false : !(0, import_es_toolkit3.isEqual)(getData(), (0, import_core5.formDataToObject)(defaultData.value));
    };
    ...
    renderTagStart(node) {
      ...
      const attrs = Object.keys(node.props).reduce((carry, name) => {
        ...
        if (["key", "head-key"].includes(name)) {
          ...
          return carry + ` ${name}="${(0, import_es_toolkit4.escape)(value)}"`;
        }
      }, "");
...
var Link = (0, import_vue7.defineComponent)({
  name: "Link",
  props: {
    as: {
      type: [String, Object],
  ...
  setup(props, { slots, attrs }) {
    const inFlightCount = (0, import_vue7.ref)(0);
    const hoverTimeout = (0, import_vue7.ref)(null);
    const prefetchModes = (0, import_vue7.computed)(() => {
    ...
    const cacheForValue = (0, import_vue7.computed)(() => {
      if (props.cacheFor !== 0) {
    ...
    const href = (0, import_vue7.computed)(() => mergeDataArray.value[0]);
    const data = (0, import_vue7.computed)(() => mergeDataArray.value[1]);
    const elProps = (0, import_vue7.computed)(() => {
      if (as.value === "button") {
        return { type: "button" };
      }
    ...
    const baseParams = (0, import_vue7.computed)(() => ({
      data: data.value,
      method: method.value,
    ...
    const visitParams = (0, import_vue7.computed)(() => ({
      ...baseParams.value,
      onCancelToken: props.onCancelToken,
      onBefore: props.onBefore,
      onStart: (visit) => {
    ...
    const regularEvents = {
      onClick: (event) => {
        if ((0, import_core6.shouldIntercept)(event)) {
          event.preventDefault();
          import_core6.router.visit(href.value, visitParams.value);
        }
      }
    };
...
function usePoll(interval, requestOptions = {}, options = {
  ...
  const { stop, start } = import_core7.router.poll(interval, requestOptions, {
    ...options,
    autoStart: false
  });
...
function usePrefetch(options = {}) {
  const isPrefetching = (0, import_vue9.ref)(false);
  const lastUpdatedAt = (0, import_vue9.ref)(null);
  const isPrefetched = (0, import_vue9.ref)(false);
  ...
  (0, import_vue9.onMounted)(() => {
    onPrefetchingListener = import_core8.router.on("prefetching", (e) => {
      if (e.detail.visit.url.pathname === window.location.pathname) {
        isPrefetching.value = true;
      }
    });
    onPrefetchedListener = import_core8.router.on("prefetched", (e) => {
      if (e.detail.visit.url.pathname === window.location.pathname) {
...
function useRemember(data, key2) {
  ...
  const restored = import_core9.router.restore(key2);
  const type = (0, import_vue10.isReactive)(data) ? import_vue10.reactive : import_vue10.ref;
  ...
  (0, import_vue10.watch)(
    remembered,
    (newValue) => {
      import_core9.router.remember((0, import_es_toolkit5.cloneDeep)(hasCallbacks ? data.__remember() : newValue), key2);
    },
    { immediate: true, deep: true }
  );
  return remembered;
...
var whenVisible_default = (0, import_vue11.defineComponent)({
  name: "WhenVisible",
  props: {
    data: {
      type: [String, Array]
  ...
  render() {
    ...
    if (this.$props.always || !this.loaded) {
      els.push((0, import_vue11.h)(this.$props.as));
    }
    if (!this.loaded) {
