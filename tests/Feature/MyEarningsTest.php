...
it('displays staff earnings and payout history', function () {
    ...
    // Create a patient for visit services
    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('pendingTotal', 200.0) // 2 visits * 100.00 cost
        );
});

it('redirects if user does not have a staff profile', function () {
    // Ensure the 'Staff' role exists for other tests, but this user won't have it
    Role::firstOrCreate(['name' => RoleEnum::STAFF->value]);

    $user = User::factory()->create(); // User without staff profile or staff role

    $this->actingAs($user)
        ->get(route('staff.my-earnings.index'))
        ->assertForbidden(); // Expect a 403 Forbidden due to middleware
});
