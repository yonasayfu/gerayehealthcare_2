...
use App\Services\StaffPayout\StaffPayoutService as AdminStaffPayoutService;
use Carbon\Carbon;
...
class StaffPayoutSampleSeeder extends Seeder
{
    ...
    public function run(AdminStaffPayoutService $payoutService): void
    {
        ...
        // Ensure base records, limiting to a small number for focused seeding
        if (Staff::count() === 0) {
            \App\Models\Staff::factory()->count(3)->create(); // Limit staff to 3
        }
        if (Patient::count() === 0) {
            \App\Models\Patient::factory()->count(5)->create(); // Limit patients to 5
        }
        ...
        foreach ($staffMembers as $index => $staff) {
            // Create 1 or 2 completed visits per staff with a predictable paid/unpaid pattern
        ...
        // Process payouts for a maximum of 3 staff to create some payout history but leave others with unpaid totals
        $staffToPayout = $staffMembers->shuffle()->take(3);
