...
class DateConversionController extends Controller
{
    ...
    public function convertEthiopianToGregorian(Request $request)
    {
        ...
        try {
            ...
            // Use DateTimeFactory::toGregorian for conversion
            // This method is expected to return an array [year, month, day] in Gregorian
            $gregorianArray = DateTimeFactory::toGregorian($ethiopicDate->getYear(), $ethiopicDate->getMonth(), $ethiopicDate->getDay());
            $gregorianDate = new \DateTime("{$gregorianArray[0]}-{$gregorianArray[1]}-{$gregorianArray[2]}");
