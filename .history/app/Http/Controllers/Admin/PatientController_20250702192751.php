    ...
    public function export(Request $request)
{
    ...
    if ($type === 'pdf') {
        // TEMPORARY: This is just HTML, ideally use DomPDF for real exports
        $html = '<h1>Patient Export</h1><ul>';
        foreach ($patients as $p) {
            $html .= "<li>{$p->full_name} - {$p->email}</li>";
        }
        $html .= '</ul>';

        return response($html, 200)
            ->header('Content-Type', 'application/pdf'); // Will not *look* like a PDF yet
    }
