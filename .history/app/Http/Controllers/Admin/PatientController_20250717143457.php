...
class PatientController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (Auth::check()) {
            $user = Auth::user();
            // Ensure the staff relationship is loaded if not already
            $user->loadMissing('staff'); 
            // Check if the logged-in user has an associated staff record
            if ($user->staff) {
                $data['registered_by_staff_id'] = $user->staff->id;
            }
...
$user = App\Models\User::where('email', 'your_superadmin_email@example.com')->first();
if ($user) {
    echo "Your Superadmin User ID is: " . $user->id . "\n";
} else {
