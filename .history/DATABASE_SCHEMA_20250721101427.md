...
-- 3. staff (Reflects the actual migration used)
CREATE TABLE staff (
    id SERIAL PRIMARY KEY,
...
CREATE TABLE visit_services (
    ...
    caregiver_assignment_id INTEGER NOT NULL REFERENCES caregiver_assignments(id) ON DELETE CASCADE,
    scheduled_at TIMESTAMP,
    check_in_time TIMESTAMP,
    check_out_time TIMESTAMP,
    check_in_latitude NUMERIC(10, 7) NULL,
    check_in_longitude NUMERIC(10, 7) NULL,
    check_out_latitude NUMERIC(10, 7) NULL,
    check_out_longitude NUMERIC(10, 7) NULL,
...
CREATE TABLE invoice_items (
    ...
    visit_service_id INTEGER NOT NULL REFERENCES visit_services(id) ON DELETE CASCADE,
...
-- 20. payout_visit_service (Pivot table for staff_payouts and visit_services)
CREATE TABLE payout_visit_service (
    staff_payout_id INTEGER NOT NULL REFERENCES staff_payouts(id) ON DELETE CASCADE,
    visit_service_id INTEGER NOT NULL REFERENCES visit_services(id) ON DELETE CASCADE,
...
- **staff ↔ patients**:
    ...
    - Via `staff_availabilities` (One-to-Many)
    - Via `staff_payouts` (One-to-Many)
    - Via `leave_requests` (One-to-Many)
    - Via `task_delegations` (One-to-Many)
- **patients ↔ services**:
    - Via `visit_services`
    - Via `invoices`
- **patients ↔ invoices** (One-to-Many)
- **invoices ↔ invoice_items** (One-to-Many)
- **visit_services ↔ invoice_items** (One-to-One)
- **staff_payouts ↔ visit_services** (Many-to-Many via `payout_visit_service`)
- **patients ↔ insurance_claims** (One-to-Many)
- **staff ↔ task_delegations** (One-to-Many)
- **visit_services ↔ task_delegations** (Optional One-to-One/Many)
...
- **roles ↔ users** (Many-to-Many via `model_has_roles`)
- **permissions ↔ roles** (Many-to-Many via `role_has_permissions`)
- **permissions ↔ users** (Many-to-Many via `model_has_permissions`)
- **notifications ↔ users/staff/patients** (Polymorphic One-to-Many)
