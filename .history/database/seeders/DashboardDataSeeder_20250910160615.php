...
class DashboardDataSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // 3) Create completed visits with check-in and check-out times for the Top Staff by Hours calculation
        $completedVisitCount = 20;
        for ($i = 0; $i < $completedVisitCount; $i++) {
            $ts = Carbon::createFromTimestamp(random_int($startOfMonth->timestamp, $end->timestamp));
            $scheduled = $ts->copy()->setTime(random_int(8, 18), random_int(0, 59));

            // Create check-in and check-out times
            $checkIn = $scheduled->copy()->addMinutes(random_int(5, 30));
            $checkOut = $checkIn->copy()->addMinutes(random_int(30, 120)); // 30 minutes to 2 hours visit duration
        ...
        // 4) Create invoices for a subset of visits (some paid, some unpaid)
        ...
        // 5) Create some insurance claims (some pending)
