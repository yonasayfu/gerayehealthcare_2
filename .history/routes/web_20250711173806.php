...
use App\Http\Controllers\Staff\LeaveRequestController as StaffLeaveRequestController; // Corrected
use App\Http\Controllers\Staff\MyAvailabilityController; // Corrected
use App\Http\Controllers\Staff\MyEarningsController; // Corrected
use App\Http\Controllers\Staff\MyVisitController; // Corrected
...
Route::middleware(['auth', 'verified', 'role:' . RoleEnum::SUPER_ADMIN->value . '|' . RoleEnum::ADMIN->value])
    ...
    ->group(function () {
        ...
        // --- Explicitly define Admin Leave Request Index only ---
        Route::get('leave-requests', [AdminLeaveRequestController::class, 'index'])->name('leave-requests.index');
        // If you need the update route for admin leave requests, define it explicitly here:
        Route::put('leave-requests/{leave_request}', [AdminLeaveRequestController::class, 'update'])->name('leave-requests.update');
        Route::patch('leave-requests/{leave_request}', [AdminLeaveRequestController::class, 'update']); // Add patch for update
