...
// --- MODIFIED: This route now intelligently selects the dashboard based on user role ---
// --- MODIFIED: This route now directs all roles to their specific dashboard controller ---
Route::get('dashboard', function () {
    $user = Auth::user();

    // Check if the logged-in user is a Super Admin or Admin
    ...
    // Fallback for any other roles that might be added in the future
...
Route::middleware(['auth', 'verified', 'role:' . RoleEnum::SUPER_ADMIN->value . '|' . RoleEnum::ADMIN->value])
   ...
   // --- NEW: Leave Request Management for Admins ---
        Route::resource('leave-requests', AdminLeaveRequestController::class)->only(['index', 'update']);
...
Route::middleware(['auth', 'verified', 'role:' . RoleEnum::STAFF->value])
    ...
    ->group(function () {
        ...
        Route::get('my-earnings', [MyEarningsController::class, 'index'])->name('my-earnings.index');
           // --- NEW: Leave Request Management for Staff ---
