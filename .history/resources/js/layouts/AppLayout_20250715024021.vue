...
<template>
  <AppSidebarLayout :breadcrumbs="breadcrumbs">
    ...
    <div class="fixed top-4 right-4 z-50">
      <button @click="showNotifications = !showNotifications" class="relative p-2 rounded-full bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 transition">
        <Bell class="w-5 h-5 text-gray-700 dark:text-gray-200" />
        <span v-if="unreadCount > 0" class="absolute top-0 right-0 block h-3 w-3 rounded-full ring-2 ring-white bg-red-500 text-white text-xs flex items-center justify-center">
          {{ unreadCount }}
        </span>
      </button>
