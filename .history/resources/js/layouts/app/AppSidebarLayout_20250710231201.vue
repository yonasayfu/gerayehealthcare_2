...
const allAdminNavItems = [
  { group: 'Patient Management', icon: UserPlus, items: [ { title: 'Dashboard', routeName: 'dashboard', icon: LayoutGrid }, { title: 'Patients', routeName: 'admin.patients.index', icon: UserPlus, permission: 'view patients' }, { title: 'Caregiver Assignments', routeName: 'admin.assignments.index', icon: CalendarClock, permission: 'view assignments' }, { title: 'Visit Services', routeName: 'admin.visit-services.index', icon: Stethoscope, permission: 'view visits' }, ] },
  communicationNavGroup,
  { group: 'Administrative Tools', icon: UserCog, items: [ { title: 'Staff', routeName: 'admin.staff.index', icon: UserCog, permission: 'view staff' }, { title: 'Staff Availability', routeName: 'admin.staff-availabilities.index', icon: CalendarCheck, permission: 'view staff' }, { title: 'Staff Payouts', routeName: 'admin.staff-payouts.index', icon: DollarSign }, ] },
  { group: 'Integrations', icon: Globe2, items: [] },
  { group: 'System Management', icon: Settings, superAdminOnly: true, items: [ { title: 'Role Management', routeName: 'admin.roles.index', icon: Users, permission: 'manage roles' }, { title: 'User Management', routeName: 'admin.users.index', icon: UserCog, permission: 'manage users' }, ] }
];

const mainNavItems = computed(() => {
    if (isSuperAdmin.value) return allAdminNavItems;
    if (isAdmin.value) return allAdminNavItems.filter(group => !group.superAdminOnly);
    if (isStaff.value) {
        return [
            { group: 'My Tools', icon: UserCheck, items: [ { title: 'Dashboard', routeName: 'dashboard', icon: LayoutGrid }, { title: 'My Visits', routeName: 'staff.my-visits.index', icon: Stethoscope }, { title: 'My Availability', routeName: 'staff.my-availability.index', icon: UserCheck } ] },
            communicationNavGroup,
        ];
    }
    return [];
...
<template>
  <!-- 
    This is the main flexbox container that fixes the responsiveness.
    It arranges the sidebar and the main content area side-by-side.
  -->
  <div class="flex h-screen bg-gray-100 dark:bg-gray-900">
    ...
    <!-- 
      ...
      'flex-1' makes it take up all available space next to the sidebar.
      'overflow-hidden' prevents the whole page from scrolling horizontally.
    -->
