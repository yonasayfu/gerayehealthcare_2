...
// Use composable only for export (keep a single source for CSV export logic).
const { exportData } = useExport({ routeName: 'admin.inventory-transactions', filters: props.filters });

// Local print helpers — open server-rendered print routes so the server-side UI template (consistent with staff/patient modules) is used.
const printCurrentView = () => {
  const params: Record<string, any> = {
    ...filters.value,
    search: search.value,
    per_page: perPage.value,
    // include current page so "Print Current" reflects visible page
    page: (props.filters as any).page || 1,
  };

  const url = route('admin.inventory-transactions.printCurrent', params);
  window.open(url, '_blank');
};

const printAllRecords = () => {
  // For "Print All" request the server-rendered all-records template.
  // We include filter criteria (search/sort) but omit page/per_page so backend returns all matching records.
  const params: Record<string, any> = {
  ...
  const url = route('admin.inventory-transactions.printAll', params);
  window.open(url, '_blank');
};
</script>
