...
const props = defineProps<{ partnerAgreements: any; filters: any }>()
...
watch([search, sortField, sortDirection, perPage], debounce(() => {
  router.get('/dashboard/partner-agreements', {
    search: search.value,
...
async function destroy(id: number) {
  const ok = await confirmDialog({
    title: 'Delete Partner Agreement',
    message: 'Are you sure you want to delete this partner agreement?',
    confirmText: 'Delete',
    cancelText: 'Cancel',
  })
  if (!ok) return
  router.delete(route('admin.partner-agreements.destroy', id), {
    preserveScroll: true,
  })
}

function exportCsv() {
  window.open(route('admin.partner-agreements.export', { type: 'csv' }), '_blank');
}
function exportData(type: 'csv', preview: boolean = false) {
  ...
  window.open(route('admin.partner-agreements.export', params), '_blank');
}
...
const printAllPartnerAgreements = () => {
  window.open(route('admin.partner-agreements.printAll', { preview: true }), '_blank');
};
    ...
    <div class="space-y-6 p-6 print:p-0 print:space-y-0">
        ...
        <div class="liquidGlass-content flex items-center justify-between p-4 gap-4">
          <div class="flex items-center gap-4">
            <div class="print:hidden">
              <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">Partner Agreements</h1>
              <p class="text-sm text-gray-600 dark:text-gray-300">Manage partner agreements</p>
            </div>
          ...
          <div class="flex items-center gap-2 print:hidden">
            <Link :href="route('admin.partner-agreements.create')" class="btn-glass">
              <span>Add Partner Agreement</span>
            </Link>
      ...
      <div class="flex flex-col md:flex-row justify-between items-center gap-4 print:hidden">
        ...
        <div class="search-glass relative w-full md:w-1/3">
          <input
            v-model="search"
            type="text"
            placeholder="Search partner agreements..."
      ...
      <div class="overflow-x-auto bg-white dark:bg-gray-900 shadow rounded-lg print:shadow-none print:rounded-none print:bg-transparent">
        <div class="hidden print:block text-center mb-4 print:mb-2 print-header-content">
            ...
            <p class="text-gray-600 dark:text-gray-400 print-document-title">Partner Agreement List (Current View)</p>
        ...
        <table class="w-full text-left text-sm text-gray-800 dark:text-gray-200 print-table">
          <thead class="bg-gray-100 dark:bg-gray-800 text-xs uppercase text-muted-foreground print-table-header">
            <tr>
              ...
              <th class="px-6 py-3 cursor-pointer" @click="toggleSort('agreement_title')">
                Title <ArrowUpDown class="inline w-4 h-4 ml-1 print:hidden" />
              </th>
              <th class="px-6 py-3 cursor-pointer" @click="toggleSort('agreement_type')">
          ...
          <tbody>
            <tr v-for="(agreement, i) in partnerAgreements.data" :key="agreement.id" class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 print-table-row">
              <td class="px-6 py-4">{{ (partnerAgreements.from ?? 1) + i }}</td>
              <td class="px-6 py-4">{{ agreement.agreement_title }}</td>
              ...
              <td class="px-6 py-4 text-right print:hidden">
                <div class="inline-flex items-center justify-end space-x-2">
                  ...
                  <Link
                    :href="route('admin.partner-agreements.edit', agreement.id)"
            ...
            <tr v-if="partnerAgreements.data.length === 0">
              <td colspan="8" class="text-center px-6 py-4 text-gray-400">No partner agreements found.</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <Pagination v-if="partnerAgreements.data.length > 0" :links="partnerAgreements.links" class="mt-6 flex justify-center print:hidden" />
      <p v-if="partnerAgreements.total" class="mt-2 text-center text-sm text-gray-600 dark:text-gray-300 print:hidden">
        Showing {{ partnerAgreements.from || 0 }}–{{ partnerAgreements.to || 0 }} of {{ partnerAgreements.total }}
      </p>
