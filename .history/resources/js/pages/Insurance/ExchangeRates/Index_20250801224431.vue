...
const props = defineProps<{
  exchangeRates: ExchangeRatePagination;
  filters: {
    search?: string;
    sort?: string;
    direction?: 'asc' | 'desc';
...
const breadcrumbs = [
  ...
  { title: 'Insurance', href: route('admin.exchange-rates.index') },
  { title: 'Exchange Rates', href: route('admin.exchange-rates.index') },
]
...
watch([search, sortField, sortDirection, perPage], debounce(() => {
  ...
  router.get(route('admin.exchange-rates.index'), params, {
    preserveState: true,
    replace: true,
  })
}, 500))
...
function exportData(type: 'csv' | 'pdf') {
  window.open(route('admin.exchange-rates.export', { type }), '_blank');
}
...
const printAllRates = () => {
    window.open(route('admin.exchange-rates.printAll', { type: 'pdf' }), '_blank');
};
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="space-y-6 p-6 print:p-0 print:space-y-0">
      ...
      <div class="rounded-lg bg-muted/40 p-4 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4 print:hidden">
        <div>
          <h1 class="text-xl font-semibold text-gray-800 dark:text-white">Exchange Rates</h1>
          <p class="text-sm text-muted-foreground">Manage all exchange rates here.</p>
        </div>
        <div class="flex flex-wrap gap-2">
          <Link :href="route('admin.exchange-rates.create')" class="inline-flex items-center gap-2 bg-cyan-600 hover:bg-cyan-700 text-white text-sm px-4 py-2 rounded-md transition">
            + Add Rate
          </Link>
          ...
          <button @click="printAllRates" class="inline-flex items-center gap-1 text-sm px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-md dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-200">
      ...
      <div class="overflow-x-auto bg-white dark:bg-gray-900 shadow rounded-lg print:shadow-none print:rounded-none print:bg-transparent">
        <div class="hidden print:block text-center mb-4 print:mb-2 print-header-content">
            ...
            <p class="text-gray-600 dark:text-gray-400 print-document-title">Exchange Rates List (Current View)</p>
        ...
        <table class="w-full text-left text-sm text-gray-800 dark:text-gray-200 print-table">
          <thead class="bg-gray-100 dark:bg-gray-800 text-xs uppercase text-muted-foreground print-table-header">
            <tr>
              <th class="px-6 py-3 cursor-pointer" @click="toggleSort('currency_code')">
                Currency Code <ArrowUpDown class="inline w-4 h-4 ml-1 print:hidden" />
              </th>
              <th class="px-6 py-3 cursor-pointer" @click="toggleSort('rate_to_etb')">
                Rate to ETB <ArrowUpDown class="inline w-4 h-4 ml-1 print:hidden" />
              </th>
              <th class="px-6 py-3 cursor-pointer" @click="toggleSort('source')">
                Source <ArrowUpDown class="inline w-4 h-4 ml-1 print:hidden" />
              </th>
              <th class="px-6 py-3 cursor-pointer" @click="toggleSort('date_effective')">
                Date Effective <ArrowUpDown class="inline w-4 h-4 ml-1 print:hidden" />
              </th>
              <th class="px-6 py-3 text-right print:hidden">Actions</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="rate in exchangeRates.data" :key="rate.id" class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 print-table-row">
              <td class="px-6 py-4">{{ rate.currency_code }}</td>
              <td class="px-6 py-4">{{ rate.rate_to_etb ?? '-' }}</td>
              <td class="px-6 py-4">{{ rate.source ?? '-' }}</td>
              <td class="px-6 py-4">{{ rate.date_effective ?? '-' }}</td>
              <td class="px-6 py-4 text-right print:hidden">
                <div class="inline-flex items-center justify-end space-x-2">
                  <Link
                    :href="route('admin.exchange-rates.show', rate.id)"
                  ...
                  <Link
                    :href="route('admin.exchange-rates.edit', rate.id)"
                  ...
                  <button @click="destroy(rate.id)" class="text-red-600 hover:text-red-800 inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-red-100 dark:hover:bg-red-900" title="Delete">
                    <Trash2 class="w-4 h-4" />
                  </button>
                </div>
              </td>
            </tr>
            <tr v-if="exchangeRates.data.length === 0">
              <td colspan="5" class="text-center px-6 py-4 text-gray-400">No exchange rates found.</td>
            </tr>
          </tbody>
        </table>
      </div>

      <Pagination v-if="exchangeRates.data.length > 0" :links="exchangeRates.links" class="mt-6 flex justify-center print:hidden" />
...
@media print {
  ...
  .print-table th:nth-child(1), .print-table td:nth-child(1) { width: 18%; } /* Currency Code */
  .print-table th:nth-child(2), .print-table td:nth-child(2) { width: 12%; } /* Rate to ETB */
  .print-table th:nth-child(3), .print-table td:nth-child(3) { width: 15%; } /* Source */
  .print-table th:nth-child(4), .print-table td:nth-child(4) { width: 8%; }  /* Date Effective */
