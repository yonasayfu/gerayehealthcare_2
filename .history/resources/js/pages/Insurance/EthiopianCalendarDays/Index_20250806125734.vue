...
async function getCurrentEthiopianDate() {
    try {
        const response = await axios.post('/api/v1/convert-to-ethiopian', { date: new Date().toISOString().split('T')[0] });
        currentEthiopianDate.value = response.data.ethiopian_date;
    } catch (error) {
        console.error(error);
    }
}
    ...
    <div class="space-y-6 p-6 print:p-0 print:space-y-0">
      ...
      <div class="rounded-lg bg-muted/40 p-4 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4 print:hidden">
        ...
        <div class="text-sm text-muted-foreground">
            Today's Date: {{ new Date().toLocaleDateString() }} (Gregorian) / {{ currentEthiopianDate }} (Ethiopian)
        </div>
      ...
      <CalendarView :holidays="ethiopianCalendarDays.data.filter(day => day.is_holiday).map(day => ({ date: day.gregorian_date, name: day.description }))" />
    </div>
  </AppLayout>
</template>
