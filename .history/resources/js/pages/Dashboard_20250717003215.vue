          ...
          <div class="p-5">
            <div v-if="upcomingVisits && upcomingVisits.length > 0" class="overflow-x-auto">
              <table class="min-w-full text-left text-sm text-gray-300">
                <thead class="font-medium bg-gray-800 text-gray-200">
                  <tr>
                ...
                <tbody>
                  <tr v-for="visit in upcomingVisits" :key="visit.id" class="border-t border-gray-700">
                    <td class="p-3">{{ visit.patient?.full_name || 'N/A' }}</td>
                    <td class="p-3">{{ formatDate(visit.scheduled_at) }}</td>
                    <td class="p-3">{{ visit.patient?.address || 'N/A' }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
