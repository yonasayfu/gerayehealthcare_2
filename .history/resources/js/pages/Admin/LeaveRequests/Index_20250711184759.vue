...
const updateRequestStatus = () => {
  ...
  form.put(route('admin.admin-leave-requests.update', requestIdToUpdate), {
    preserveScroll: true,
    onSuccess: () => {
      console.log('Leave request update successful!');
      isDialogOpen.value = false;
      form.reset();

      // Find the updated request in the current data and update its status and notes
      const updatedRequestIndex = props.leaveRequests.data.findIndex(
        (req) => req.id === requestIdToUpdate
      );

      if (updatedRequestIndex !== -1) {
        props.leaveRequests.data[updatedRequestIndex].status = form.status;
        props.leaveRequests.data[updatedRequestIndex].admin_notes = form.admin_notes;
      }

      // Re-fetch filters if needed, or rely on the direct update above
      // If direct update causes issues or you need other parts of the page to react,
      // consider a partial reload here:
      // router.reload({ only: ['leaveRequests', 'filters'] });
    },
    onError: (errors) => {
      ...
      <div class="p-5">
        ...
        <div v-if="leaveRequests.data.length > 0" class="overflow-x-auto">
          <table class="min-w-full text-left text-sm">
            <thead>
              <tr>
                <th class="p-2 cursor-pointer" @click="sortColumn('staff_name')">
                  <div class="flex items-center">
                    Staff Member
                    <template v-if="currentSortBy === 'staff_name'">
                      <ChevronUp class="h-4 w-4 ml-1" v-if="currentSortOrder === 'asc'" />
                      <ChevronDown class="h-4 w-4 ml-1" v-else />
                    </template>
                  </div>
                </th>
                <th class="p-2 cursor-pointer" @click="sortColumn('start_date')">
                  <div class="flex items-center">
                    Dates Requested
