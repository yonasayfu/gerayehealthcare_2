...
const updateRequestStatus = () => {
  ...
  form.put(route('admin.admin-leave-requests.update', selectedRequest.value.id), {
    preserveScroll: true,
    onSuccess: (page) => {
      console.log('Inertia PUT request SUCCESS! Laravel responded with a redirect.');
      console.log('Page props after success (from new GET request):', page.props); // Examine the props Inertia received after redirect

      isDialogOpen.value = false; // Close the dialog
      form.reset(); // Reset form fields

      // *** REMOVED: applyFilters() call here ***
      // Inertia handles the redirect and re-fetches props automatically.
      // Manually calling applyFilters here was causing a conflict or race condition.
    },
    onError: (errors) => {
    ...
    onFinish: () => {
      console.log('Inertia PUT request finished (success or failure, and any subsequent redirect handled).');
    }
  });
};
