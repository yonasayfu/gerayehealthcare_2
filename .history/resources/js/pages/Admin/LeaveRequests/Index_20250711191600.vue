...
const openUpdateDialog = (request: LeaveRequest, status: 'Approved' | 'Denied') => {
  console.log('openUpdateDialog called for request:', request.id, 'status:', status);
...
const updateRequestStatus = () => {
  ...
  const updateRoute = route('admin.admin-leave-requests.update', selectedRequest.value.id);
  console.log('Attempting to submit form to URL:', updateRoute); // Log the URL

  form.put(updateRoute, {
    preserveScroll: true,
    onSuccess: () => {
      console.log('Leave request update successful! Triggering data reload...');
