...
import { Head, Link, useForm, usePage, router } from '@inertiajs/vue3'; // Import router
...
import { ChevronUp, ChevronDown } from 'lucide-vue-next'; // Import icons for sorting
...
const applyFilters = (options: { preserveState?: boolean; preserveScroll?: boolean } = {}) => {
  router.get(
    ...
    {
      preserveState: options.preserveState !== undefined ? options.preserveState : true,
      preserveScroll: options.preserveScroll !== undefined ? options.preserveScroll : true,
...
const sortColumn = (column: string) => {
  // Map friendly frontend column names to backend column names
  const backendColumnMap: Record<string, string> = {
    'staff_member': 'staff_first_name', // Sort by first name of staff
    'dates_requested': 'start_date',
    'status': 'status',
    // Add other sortable columns here if needed
  };

  const backendColumn = backendColumnMap[column] || column; // Use mapped name, or original if not found

  if (currentSortBy.value === backendColumn) {
    // Toggle sort order if clicking the same column
    currentSortOrder.value = currentSortOrder.value === 'asc' ? 'desc' : 'asc';
  } else {
    // Default to asc if sorting a new column
    currentSortBy.value = backendColumn;
...
const updateRequestStatus = () => {
  ...
  const requestIdToUpdate = selectedRequest.value.id;

  form.put(route('admin.admin-leave-requests.update', requestIdToUpdate), {
    preserveScroll: true,
    // On success, instead of manually updating the array,
    // trigger a full Inertia reload of the necessary props.
    // This is more reliable for ensuring the data is fresh from the server.
    onSuccess: () => {
      console.log('Leave request update successful! Reloading data...');
      isDialogOpen.value = false;
      form.reset();

      // Trigger a reload of the current page with the same filters
      // This forces Inertia to re-fetch 'leaveRequests' and 'filters' from the server
      applyFilters({ preserveState: false, preserveScroll: true }); // Ensure state is reset and then re-fetched
    },
    onError: (errors) => {
        console.error('Error updating leave request:', errors);
        // You might want to display these errors to the user, e.g., using a toast
    }
  });
};
      ...
      <div class="p-5">
        ...
        <div v-if="leaveRequests.data.length > 0" class="overflow-x-auto">
          <table class="min-w-full text-left text-sm">
            <thead>
              <tr>
                <th class="p-2 cursor-pointer" @click="sortColumn('staff_member')">
                ...
                <th class="p-2 cursor-pointer" @click="sortColumn('dates_requested')">
                  <div class="flex items-center">
                    Dates Requested
                ...
                <th class="p-2 cursor-pointer" @click="sortColumn('status')">
                  <div class="flex items-center">
                    Status
