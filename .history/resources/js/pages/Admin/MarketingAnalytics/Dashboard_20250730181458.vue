...
const props = defineProps({
  dashboardStats: { type: Object, required: true },
  campaignPerformanceData: { type: Array, required: true },
  trafficSourceData: { type: Array, required: true },
  conversionFunnelData: { type: Object, required: true },
});
...
const campaignBarChartData = computed(() => ({
  labels: props.campaignPerformanceData.map((item: any) => item.date),
  datasets: [
    {
      label: 'Impressions',
      backgroundColor: '#3b82f6',
      data: props.campaignPerformanceData.map((item: any) => item.impressions),
    },
    {
      label: 'Clicks',
      backgroundColor: '#66BB6A',
      data: props.campaignPerformanceData.map((item: any) => item.clicks),
    },
  ],
}));

const trafficPieChartData = computed(() => ({
  labels: props.trafficSourceData.map((item: any) => item.utm_source),
  datasets: [
    {
      backgroundColor: ['#42A5F5', '#66BB6A', '#FFA726', '#EF5350', '#9C27B0', '#FFC107'],
      data: props.trafficSourceData.map((item: any) => item.lead_count),
    },
  ],
}));
...
const getConversionFunnelPercentage = (step: string) => {
  const total = Object.values(props.conversionFunnelData).reduce((sum: any, value: any) => sum + value, 0);
  return total > 0 ? ((props.conversionFunnelData as any)[step] / total) * 100 : 0;
};
</script>
