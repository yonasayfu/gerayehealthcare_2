...
function animateProgressBars() {
  document.querySelectorAll<HTMLElement>('.progress-bar').forEach(bar => {
    ...
    if (targetWidth) {
      ...
      // The original `animateValue` function is called directly in onMounted for individual counters
      // For progress bars, the width transition itself is the animation, and the percentage is updated via data
    }
  });

  // Re-animate the percentage counters for traffic sources specifically
  props.trafficSourceData.forEach((data, index) => {
    const total = props.trafficSourceData.reduce((sum, item) => sum + item.lead_count, 0);
    const percent = total > 0 ? (data.lead_count / total) * 100 : 0;
    const counterElementId = `traffic-source-percent-${index}`;
    animateValue(counterElementId, 0, parseFloat(percent.toFixed(1)), 1500);
  });
}
...
// Helper function to get distinct background colors for progress bars
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 transform transition duration-500 hover:scale-[1.02] card-hover animate-fadeIn" style="animation-delay: 1.1s">
          ...
          <div class="space-y-3 mt-4">
            <div v-for="(data, index) in props.trafficSourceData" :key="index" class="flex items-center justify-between">
              <span class="w-24 font-medium text-gray-700">{{ data.utm_source }}</span>
              <div class="flex-1 mx-4">
                <div class="w-full bg-gray-200 rounded-full h-2.5">
                  <div
                    class="h-2.5 rounded-full progress-bar"
                    :style="{ width: (data.lead_count / (props.trafficSourceData.length > 0 ? props.trafficSourceData.reduce((sum, item) => sum + item.lead_count, 0) : 1) * 100) + '%' }"
                    :data-width="(data.lead_count / (props.trafficSourceData.length > 0 ? props.trafficSourceData.reduce((sum, item) => sum + item.lead_count, 0) : 1) * 100) + '%'"
                    :class="getProgressBarColor(index)"
                  ></div>
                </div>
              </div>
              <span class="w-10 text-right font-medium text-gray-800" :id="`traffic-source-percent-${index}`">
                {{ (data.lead_count / (props.trafficSourceData.length > 0 ? props.trafficSourceData.reduce((sum, item) => sum + item.lead_count, 0) : 1) * 100).toFixed(1) }}%
              </span>
            </div>
          </div>
        </div>
