...
// NEW: Conversion Funnel Bar Chart Data (using vue-chartjs)
const conversionFunnelBarChartData = computed(() => ({
  labels: ['New', 'Contacted', 'Qualified', 'Converted'],
  datasets: [
    {
      label: 'Leads Count',
      backgroundColor: '#3B82F6', // A consistent blue for funnel stages
      data: [
        safeConversionFunnelData.value.New,
        safeConversionFunnelData.value.Contacted,
        safeConversionFunnelData.value.Qualified,
        safeConversionFunnelData.value.Converted,
      ],
    },
  ],
}));
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 transform transition duration-500 hover:scale-[1.02] card-hover animate-fadeIn" style="animation-delay: 1.2s">
          ...
          <div class="h-[300px] mt-4">
            <Bar :data="conversionFunnelBarChartData" :options="chartOptions" />
          </div>
        </div>
      </div>
...
/* Alpine.js chart specific styles - these are no longer directly relevant for the funnel chart */
.line {
