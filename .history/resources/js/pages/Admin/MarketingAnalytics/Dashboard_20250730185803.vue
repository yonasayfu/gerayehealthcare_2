...
import AppLayout from '@/layouts/AppLayout.vue'; // Standard AppLayout from admin dashboard reference
...
import StatCard from '@/components/StatCard.vue'; // Assuming this component exists from admin dashboard reference
...
// Interfaces from the original (first) template
interface DashboardStats {
  totalLeads: number;
  convertedLeads: number;
  conversionRate: number;
  totalMarketingSpend: number; // Changed to number for calculations
...
const currentTab = ref('Overview'); // State for active tab, assuming tabs for navigation

function handleTabChange(tab: string) {
  currentTab.value = tab;
}

// Reactive data initialized from props or defaults
...
// Counter animations (from original/second template)
...
// Animate progress bars (from original/second template)
function animateProgressBars() {
  document.querySelectorAll<HTMLElement>('.progress-bar').forEach(bar => {
    ...
    if (targetWidth) {
      ...
      // Find the corresponding span element by ID (e.g., 'traffic-source-percent-0')
      const index = parseInt(bar.dataset.index || '0');
      const counterElement = document.getElementById(`traffic-source-percent-${index}`) as HTMLElement;
...
onMounted(() => {
  // Animate dashboard counters from the first template's data
...
const campaignBarChartData = computed(() => ({
  ...
  datasets: [
    {
      label: 'Impressions',
      // Using admin dashboard consistent blue
      backgroundColor: '#3b82f6', // Tailwind blue-500/600
      data: campaignPerformanceData.value.map((item: CampaignPerformanceItem) => item.impressions),
    },
    {
      label: 'Clicks',
      // Using admin dashboard consistent green
      backgroundColor: '#66BB6A', // Tailwind green-500/600
...
const trafficPieChartData = computed(() => ({
  ...
  datasets: [
    {
      // Colors from admin dashboard StatCard examples (blue, green, yellow, red, purple, orange)
...
// Data fetching functions (as in admin dashboard reference)
...
watch(currentTab, (newTab) => {
  if (newTab === 'Overview' || newTab === 'Analytics') { // Assuming 'Overview' and 'Analytics' are the relevant tabs
    fetchDashboardData();
      ...
      <div v-if="currentTab === 'Overview' || currentTab === 'Analytics'">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 transform transition duration-500 hover:scale-[1.02] card-hover animate-fadeIn" style="animation-delay: 1.1s">
            ...
            <div class="space-y-3 mt-4">
              <div v-for="(data, index) in trafficSourceData" :key="index" class="flex items-center justify-between">
                ...
                <span class="w-10 text-right font-medium" :id="`traffic-source-percent-${index}`">0%</span>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 transform transition duration-500 hover:scale-[1.02] card-hover animate-fadeIn" style="animation-delay: 1.2s">
            ...
            <div class="space-y-4">
              <div v-for="(value, key) in conversionFunnelData" :key="key" class="flex justify-between items-center p-3 rounded-lg"
                   :class="{
                     'bg-blue-50': key === 'New',
                     'bg-green-50': key === 'Contacted',
                     'bg-yellow-50': key === 'Qualified',
                     'bg-indigo-50': key === 'Converted',
                   }">
...
<style scoped>
/* Animations from original/second template */
@keyframes fadeIn {
