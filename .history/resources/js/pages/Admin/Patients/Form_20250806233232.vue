  ...
  <form @submit.prevent="emit('submit')">
    <div class="border-b border-gray-900/10 pb-12">
      ...
      <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
        ...
        <div class="sm:col-span-3">
          <label class="block text-sm font-medium text-blue-900 bg-green-200:text-white">Employer (Corporate Client)</label>
          <div class="mt-2">
            <select
              v-model="form.corporate_client_id"
              class="shadow-sm border border-gray-300 text-blue-900 dark:text-white sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5"
            >
              <option :value="null">Select Employer</option>
              <option v-for="client in corporateClients" :key="client.id" :value="client.id">{{ client.name }}</option>
            </select>
