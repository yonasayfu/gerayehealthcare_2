...
const breadcrumbs = [
  ...
  { title: 'Patients', href: route('admin.patients.index') }, // CORRECTED
]
...
watch([search, sortField, sortDirection, perPage], debounce(() => {
  router.get(route('admin.patients.index'), { // CORRECTED
    search: search.value,
...
function destroy(id: number) {
  if (confirm('Are you sure you want to delete this patient?')) {
    router.delete(route('admin.patients.destroy', id)) // CORRECTED
  }
}

function exportData(type: 'csv' | 'pdf') {
  // This was a raw URL, which is fine, but using the route() helper is better practice.
  window.open(route('admin.patients.export', { type }), '_blank'); // CORRECTED
}
    ...
    <div class="space-y-6 p-6">
      ...
      <div class="rounded-lg bg-muted/40 p-4 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-2">
          <Link :href="route('admin.patients.create')" class="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white text-sm px-4 py-2 rounded-md transition"> <!-- CORRECTED -->
            + Add Patient
          </Link>
            ...
            <tr v-for="patient in patients.data" :key="patient.id" class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
              ...
              <td class="px-6 py-4 text-right">
                <div class="inline-flex items-center justify-end space-x-2">
                  <Link
                    :href="route('admin.patients.edit', patient.id)" <!-- CORRECTED -->
