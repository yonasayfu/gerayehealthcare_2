...
// Debounced async check for uniqueness
const checkFieldUnique = debounce(async (field, value) => {
  if (!value) {
    localErrors[field] = ''
    return
  }
  ...
  try {
    ...
    localErrors[field] = data.valid ? '' : `${field === 'email' ? 'Email' : 'Phone number'} already exists.`
  } catch (err) {
    localErrors[field] = 'Validation failed.'
  }
}, 400)
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="p-6 space-y-6">
      ...
      <div class="rounded-lg border border-border bg-white dark:bg-background p-6 shadow-sm space-y-6">
        <!-- Inject form + localErrors to form component -->
