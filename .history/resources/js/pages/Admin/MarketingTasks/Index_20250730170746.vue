...
function printCurrentView() {
    const params = {
        ...
        related_content_id: relatedContentId.value,
    };
...
function clearFilters() {
  ...
  doctorId.value = '';
  status.value = '';
  scheduledAtStart.value = '';
  scheduledAtEnd.value = '';
  relatedContentId.value = '';
}
</script>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="space-y-6 p-6 print:p-0 print:space-y-0">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 print:hidden">
        ...
        <div>
          <label for="doctorId" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Doctor:</label>
          <select v-model="doctorId" id="doctorId" class="mt-1 block w-full shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 p-2.5">
            <option value="">All Doctors</option>
            <option v-for="doctor in staffs" :key="doctor.id" :value="doctor.id">{{ doctor.user?.name ?? '-' }}</option>
          </select>
        </div>

        <div>
          <label for="status" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Status:</label>
          <select v-model="status" id="status" class="mt-1 block w-full shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 p-2.5">
        ...
        <div>
          <label for="scheduledAtStart" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Scheduled At (Start):</label>
          <TextInput
            type="date"
            v-model="scheduledAtStart"
            id="scheduledAtStart"
        ...
        <div>
          <label for="scheduledAtEnd" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Scheduled At (End):</label>
          <TextInput
            type="date"
            v-model="scheduledAtEnd"
            id="scheduledAtEnd"
            class="mt-1 block w-full"
          />
        </div>
        ...
        <div>
          <label for="relatedContentId" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Related Content:</label>
          <select v-model="relatedContentId" id="relatedContentId" class="mt-1 block w-full shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 p-2.5">
            <option value="">All Related Content</option>
            <option v-for="content in campaignContents" :key="content.id" :value="content.id">{{ content.title }}</option>
          </select>
        </div>
