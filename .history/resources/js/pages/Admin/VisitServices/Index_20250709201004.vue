...
import { Edit3, Trash2, ArrowUpDown, Plus, Paperclip, MapPin, FileText } from 'lucide-vue-next' // Import FileText
    ...
    <div class="space-y-6 p-6">
            ...
            <tr v-for="visit in visitServices.data" :key="visit.id" class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
              ...
              <td class="px-6 py-4 text-right">
                <div class="inline-flex items-center justify-end space-x-2">
                  <!-- New "View" Link -->
                  <Link :href="route('admin.visit-services.show', visit.id)" class="inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600" title="View Details">
      ...
      <div v-if="visitServices.links.length > 3" class="flex justify-end">
          <div class="flex items-center space-x-1">
              <Link v-for="(link, i) in visitServices.links" :key="i" :href="link.url || ''" v-html="link.label" :class="['px-3 py-1 rounded-md text-sm', link.active ? 'bg-indigo-600 text-white' : 'hover:bg-gray-200 dark:hover:bg-gray-700', !link.url && 'cursor-not-allowed text-gray-400']" />
          </div>
      </div>
    </div>
  </AppLayout>
