...
const openEditModal = (availability) => {
    ...
    form.start_time = formatToDateTimeLocal(availability.start_time);
    form.end_time = formatToDateTimeLocal(availability.end_time);
    showModal.value = true;
};

const submit = () => {
    // Create a temporary data object to hold the converted dates
    const data = {
        ...form.data(),
    ...
    if (isEditMode.value) {
        router.put(route('admin.staff-availabilities.update', form.id), data, { onFinish });
    } else {
        router.post(route('admin.staff-availabilities.store'), data, { onFinish });
    }
};
