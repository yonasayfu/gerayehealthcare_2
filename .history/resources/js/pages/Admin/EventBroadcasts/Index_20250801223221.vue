...
function destroy(id) {
    if (confirm('Are you sure you want to delete this event broadcast?')) {
        router.delete(route('admin.event-broadcasts.destroy', id));
    }
}

function exportData(type) {
    window.open(route('admin.event-broadcasts.export', { type }), '_blank');
}
...
const printAllBroadcasts = () => {
    window.open(route('admin.event-broadcasts.export', { type: 'pdf' }), '_blank');
};
    ...
    <AdminLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-6 p-6 print:p-0 print:space-y-0">
            <div class="rounded-lg bg-muted/40 p-4 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4 print:hidden">
                <div>
                    <h1 class="text-xl font-semibold text-gray-800 dark:text-white">Event Broadcasts</h1>
                    <p class="text-sm text-muted-foreground">Manage all event broadcasts here.</p>
                </div>
                <div class="flex flex-wrap gap-2">
                    <Link :href="route('admin.event-broadcasts.create')" class="inline-flex items-center gap-2 bg-cyan-600 hover:bg-cyan-700 text-white text-sm px-4 py-2 rounded-md transition">
                        + Add Broadcast
                    </Link>
                    ...
                    <button @click="printAllBroadcasts" class="inline-flex items-center gap-1 text-sm px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-md dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-200">
                ...
                <table class="w-full text-left text-sm text-gray-800 dark:text-gray-200 print-table">
                    <thead class="bg-gray-100 dark:bg-gray-800 text-xs uppercase text-muted-foreground print-table-header">
                        <tr>
                            ...
                            <th class="px-6 py-3 cursor-pointer" @click="toggleSort('sent_by_staff_id')">
                                Sent By Staff ID <ArrowUpDown class="inline w-4 h-4 ml-1 print:hidden" />
                            </th>
                    ...
                    <tbody>
                        <tr v-for="broadcast in broadcasts.data" :key="broadcast.id" class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 print-table-row">
                            <td class="px-6 py-4">{{ broadcast.event_id }}</td>
                            <td class="px-6 py-4">{{ broadcast.channel }}</td>
                            <td class="px-6 py-4">{{ broadcast.sent_by_staff_id }}</td>
                            <td class="px-6 py-4 text-right print:hidden">
                                <div class="inline-flex items-center justify-end space-x-2">
                                    ...
                                    <button @click="destroy(broadcast.id)" class="text-red-600 hover:text-red-800 inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-red-100 dark:hover:bg-red-900" title="Delete">
