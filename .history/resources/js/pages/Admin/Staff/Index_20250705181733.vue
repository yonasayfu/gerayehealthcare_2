...
const breadcrumbs = [
  ...
  { title: 'Staff', href: route('admin.staff.index') }, // CORRECTED
]
...
watch([search, sortField, sortDirection, perPage], debounce(() => {
  router.get(route('admin.staff.index'), { // CORRECTED
    search: search.value,
...
function destroy(id: number) {
  if (confirm('Are you sure you want to delete this staff member?')) {
    router.delete(route('admin.staff.destroy', id)) // CORRECTED
  }
}

function exportData(type: 'csv' | 'pdf') {
  window.open(route('admin.staff.export', { type }), '_blank'); // CORRECTED & IMPROVED
}
    ...
    <div class="space-y-6 p-6">
      ...
      <div class="rounded-lg bg-muted/40 p-4 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-2">
          <Link :href="route('admin.staff.create')" class="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white text-sm px-4 py-2 rounded-md transition"> <!-- CORRECTED -->
            + Add Staff
          </Link>
            ...
            <tr v-for="member in staff.data" :key="member.id" class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
              ...
              <td class="px-6 py-4 text-right">
                <div class="inline-flex items-center justify-end space-x-2">
                    <Link :href="route('admin.staff.show', member.id)" class="inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-blue-100 dark:hover:bg-blue-900 text-indigo-600" title="View"> <!-- CORRECTED -->
                      <Eye class="w-4 h-4" /> <!-- Changed to Eye icon for consistency -->
                    </Link>

                  <Link :href="route('admin.staff.edit', member.id)" class="inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-blue-100 dark:hover:bg-blue-900 text-blue-600" title="Edit"> <!-- CORRECTED -->
                    <Edit3 class="w-4 h-4" />
                  </Link>
