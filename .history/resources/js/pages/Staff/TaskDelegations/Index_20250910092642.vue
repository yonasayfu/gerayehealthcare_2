...
function switchScope(s: 'assigned' | 'created') {
  ...
  scope.value = s
  router.get(route('staff.task-delegations.index'), { scope: s, search: search.value, per_page: perPage.value }, { preserveState: true, replace: true })
}
...
watch([search, perPage], debounce(([searchValue, perPageValue]) => {
  router.get(
    ...
    { scope: scope.value, search: searchValue, per_page: perPageValue },
        ...
        <form @submit.prevent="submitNew" class="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <div class="md:col-span-4">
            <button
              ...
              class="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-50"
            >
              Create Task
            </button>
          </div>
