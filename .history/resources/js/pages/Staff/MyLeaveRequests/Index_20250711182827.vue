...
import { type BreadcrumbItem, type LeaveRequestsPagination } from '@/types';
...
onMounted(() => {
  // This will force Inertia to re-fetch the data for this page
  // when the component is mounted (i.e., when the page is visited/reloaded).
  // This helps ensure the staff sees the latest status updates.
  // We use preserveState: true to avoid scrolling to top and preserve form state if any.
  // However, for a full data refresh, a simple get is often sufficient.
  // For this specific case, a full page reload is desired to get fresh data.
  // If the page is already loaded and the user navigates back to it, this will ensure fresh data.
  // If the user is already on the page and an admin updates, this won't automatically update.
  // For that, WebSockets would be needed. But for ensuring fresh data on page load, this is effective.
  // No need for Inertia.reload() here, as Inertia already fetches fresh props on a full page visit.
  // The issue is likely that the user expects real-time updates without a refresh.
  // For now, let's ensure the types are correct and the route is correct.
});
