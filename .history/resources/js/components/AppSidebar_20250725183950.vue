...
// ONLY for toggling groups - nothing else should call this
...
const toggleAllGroups = () => {
  if (openGroups.value.length === mainNavItems.value.length) {
    collapseAllGroups()
  } else {
    expandAllGroups()
  }
...
const debugClick = (groupName: string, action: string) => {
  console.log(`Group: ${groupName}, Action: ${action}, Open Groups:`, [...openGroups.value])
}
</script>
        ...
        <SidebarMenu>
            <div v-for="group in mainNavItems" :key="group.group" class="mb-1">
                ...
                <div class="group/collapsible">
                    <div 
                        @click="() => { debugClick(group.group, 'toggle'); toggleGroup(group.group); }"
            ...
            <SidebarMenuItem>
                ...
                >
                    <div class="flex items-center gap-2">
                        <component 
                            :is="openGroups.length === mainNavItems.length ? Minimize2 : Maximize2" 
                            class="h-4 w-4 flex-shrink-0" 
                        />
                        <span class="truncate">{{ openGroups.length === mainNavItems.length ? 'Collapse All' : 'Expand All' }}</span>
                    </div>
                </SidebarMenuButton>
            </SidebarMenuItem>
        </SidebarMenu>
