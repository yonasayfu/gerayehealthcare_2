...
// Initialize calendar with current date or modelValue
  onMounted(async () => {
    // Initialize with a reasonable default Ethiopian date (e.g., current Gregorian year - 7, month 1)
    const initialGregorianYear = new Date().getFullYear();
    currentEthiopianYear.value = initialGregorianYear - 7; // Approximate Ethiopian year
    currentEthiopianMonth.value = 1; // Meskerem

    // Attempt to set based on modelValue first
    if (props.modelValue) {
      ...
      // If no modelValue, try to set to today's Ethiopian date
      const todayGregorian = new Date();
      const todayEthiopian = await convertGregorianToEthiopian({ year: todayGregorian.getFullYear(), month: todayGregorian.getMonth() + 1, day: todayGregorian.getDate() });
      if (todayEthiopian) {
        ...
        console.warn('Failed to get today\'s Ethiopian date, falling back to default initial values.');
      }
    }
