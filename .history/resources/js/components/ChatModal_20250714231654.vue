...
const fetchMessages = async (recipientId = null, searchQuery = '') => {
  try {
    ...
    // If no recipient was explicitly selected, and there are conversations, select the first one
    if (!selectedConversation.value && conversations.value.length > 0) {
      selectedConversation.value = conversations.value[0];
    }
    // Ensure form.receiver_id is always in sync with selectedConversation
