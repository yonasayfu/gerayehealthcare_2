...
const allAdminNavItems = [
  ...
  communicationNavGroup,
  {
    ...
    items: [
      ...
      // --- DEBUGGING: Using a direct href instead of routeName ---
        ...
        return [
            {
                ...
                items: [
                    ...
                    // --- DEBUGGING: Using a direct href instead of routeName ---
                    { title: 'My Leave Requests', href: '/dashboard/leave-requests', icon: CalendarOff }
                ]
            },
            communicationNavGroup,
        ];
            ...
            <template v-for="item in group.items" :key="item.title">
              <SidebarMenuItem v-if="!item.permission || can(item.permission)">
                <SidebarMenuButton as-child>
                  <!-- DEBUGGING: This Link now checks for item.href first -->
                  <Link :href="item.href || (item.routeName ? route(item.routeName) : '#')" class="flex items-center gap-2 px-2 py-1 text-sm">
                    <component :is="item.icon" class="h-4 w-4" />
                    <Link v-if="item.routeName && canAccessRoute(item)" :href="route(item.routeName)" class="flex-1 truncate">
                      <span v-if="isSidebarCollapsed" class="sr-only">{{ item.title }}</span>
