...
// Track open state for each group using a Map for better performance
...
// Initialize all groups as collapsed (false)
onMounted(() => {
  mainNavItems.value.forEach(group => {
    openGroups.value.set(group.group, false)
  })
})

// Toggle individual group
const toggleGroup = (groupName: string) => {
  const currentState = openGroups.value.get(groupName) || false
  openGroups.value.set(groupName, !currentState)
}

// Expand all groups
const expandAllGroups = () => {
  mainNavItems.value.forEach(group => {
    openGroups.value.set(group.group, true)
  })
    ...
    <SidebarContent>
        <SidebarMenu>
            <div v-for="group in mainNavItems" :key="group.group" class="mb-1">
                <Collapsible
                    :open="openGroups.get(group.group) || false"
                    @update:open="(isOpen) => openGroups.set(group.group, isOpen)"
                    as-child
                ...
                >
                    <SidebarMenuItem>
                        <CollapsibleTrigger as-child>
                            <SidebarMenuButton 
                                :tooltip="group.group" 
                                class="font-medium text-sidebar-foreground hover:bg-accent hover:text-accent-foreground rounded-md transition-colors duration-150 justify-between px-3 py-2 w-full flex items-center gap-2"
                            >
                        ...
                        <CollapsibleContent class="overflow-hidden data-[state=closed]:animate-collapse-up data-[state=open]:animate-collapse-down">
