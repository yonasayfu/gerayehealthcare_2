...
// Expand all groups (but limit to 2)
const expandAllGroups = () => {
  const allGroupNames = mainNavItems.value.map(group => group.group)
  openGroups.value = allGroupNames.slice(0, 2) // Only first 2 groups
...
// Watch for sidebar collapse from external button
watch(() => isSidebarCollapsed.value, (newVal) => {
  // This will trigger reactivity when sidebar is collapsed externally
  // The template handles the visual changes automatically
})
</script>
