...
console.log('GeraChat component script loaded.');
...
const emit = defineEmits(['close', 'minimize']); // 'minimize' event for parent
...
const showConversationList = ref(true); // For mobile responsiveness
...
const messageInput = ref<HTMLInputElement | null>(null); // Ref for message input for emoji insertion
...
const onEmojiSelect = (emoji: { data: string }) => {
  if (messageInput.value) {
    const start = messageInput.value.selectionStart || 0;
    const end = messageInput.value.selectionEnd || 0;
    const text = form.message;
    form.message = text.substring(0, start) + emoji.data + text.substring(end);

    // Keep cursor at the end of the inserted emoji
    nextTick(() => {
      if (messageInput.value) {
        messageInput.value.selectionEnd = start + emoji.data.length;
...
const submit = async () => {
  ...
  try {
    form.post(route('messages.store'), { // Using form.post directly here
      preserveScroll: true,
      forceFormData: true,
      onSuccess: () => {
        ...
        <div v-show="!showConversationList || currentWindowWidth >= 768"
          ...
          <template v-else>
            ...
            <div class="p-4 border-t border-border bg-card relative">
              ...
              <form @submit.prevent="submit" class="flex items-end gap-2"> <img :src="authUser.profile_photo_url || `https://ui-avatars.com/api/?name=${authUser.name}&color=7F9CF5&background=EBF4FF`" :alt="authUser.name" class="w-10 h-10 rounded-full object-cover flex-shrink-0">
                ...
                <div v-if="form.attachment" class="flex items-center text-sm bg-muted rounded-full px-3 py-1 flex-shrink-0">
                ...
                <input
                  ref="messageInput" v-model="form.message"
                  type="text"
