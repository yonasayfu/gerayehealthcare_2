...
import { ref, watch, nextTick, computed, onMounted, onUnmounted } from 'vue'; // Added onMounted, onUnmounted
...
const currentWindowWidth = ref(0); // Initialize with 0
...
const modalWidth = ref(800); // Default desktop width, will be updated onMount
const modalHeight = ref(600); // Default desktop height, will be updated onMount
...
const onMouseMove = (e: MouseEvent) => {
  ...
  const minModalHeight = currentWindowWidth.value < 768 ? currentWindowWidth.value * 0.8 : 400; // Use window.innerWidth for height for consistency on smaller views
...
watch(() => selectedConversation.value, (newVal: Conversation | null) => {
  ...
  if (newVal) {
    ...
    if (currentWindowWidth.value < 768) { // Use reactive window width
...
<template>
  <div v-if="props.isOpen" class="fixed bottom-6 right-6 z-50 chat-modal-widget"
       :class="{ 'fixed inset-0 !bottom-0 !right-0 !top-0 !left-0': currentWindowWidth < 768 }" > <div
      ref="modalRef"
