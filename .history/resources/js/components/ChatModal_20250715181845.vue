...
console.log('ChatModal component script loaded. (Attempting to fix TypeError)');
...
// --- Fetching Logic (Refined for stability and debugging) ---
const fetchMessages = async (recipientId: number | null = null, searchQuery: string = '') => {
  ...
  try {
    ...
    // Assign selectedConversation carefully, avoid unnecessary re-triggers
    if (selectedConversation.value?.id !== newSelectedConversation?.id) {
        ...
        selectedConversation.value = newSelectedConversation; // Handle initial null to non-null transition
        console.log('[fetchMessages] Selected conversation initialized to:', selectedConversation.value?.name);
    }


    // Filter out any potential null/undefined messages before assigning
...
watch(search, (value: string) => {
  ...
  // Debounce the search if it's too frequent, but let's see if this resolves the current issue first.
...
watch(() => selectedConversation.value, (newVal: Conversation | null) => {
    ...
    if (newId !== previousSelectedConversationId.value) {
        ...
        scrollToBottom(); // Scroll first, then fetch
        if (newId) {
...
const submit = async () => {
  ...
  try {
    await form.post(route('messages.store'), {
      preserveScroll: true,
      forceFormData: true,
      onSuccess: () => {
...
const selectConversation = (convoId: number) => {
  ...
  // Manually update selectedConversation. The watcher will then trigger fetchMessages.
          ...
          <template v-else>
            ...
            <div ref="chatContainer" class="flex-grow p-6 overflow-y-auto space-y-4 custom-scrollbar min-h-0">
              ...
              <div v-else
                ...
                :key="message?.id || index" v-if="message && typeof message === 'object'" class="flex"
