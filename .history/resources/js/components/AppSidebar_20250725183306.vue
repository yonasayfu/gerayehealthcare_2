...
const toggleGroup = (groupName: string) => {
  ...
  if (currentIndex > -1) {
    ...
    if (openGroups.value.length >= 2) {
      // Remove the first (oldest) open group ONLY if we're not in "expand all" mode
...
const toggleAllGroups = () => {
  if (areAllGroupsExpanded.value) {
    ...
    // Currently collapsed, expand all (but limit to 2 if that's the rule)
    const allGroupNames = mainNavItems.value.map(group => group.group)
    openGroups.value = allGroupNames // Open all groups
