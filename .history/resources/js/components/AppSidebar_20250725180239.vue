                          ...
                          <template v-for="item in group.items" :key="item.title">
                            <SidebarMenuItem v-if="!item.permission || can(item.permission)">
                              <SidebarMenuButton as-child>
                                <Link :href="item.routeName ? route(item.routeName) : '#'" class="flex items-center gap-2 px-2 py-1 text-sm hover:bg-muted/30 rounded-md w-full" preserve-scroll preserve-state>
