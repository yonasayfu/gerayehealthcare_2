...
<template>
  <Sidebar collapsible="icon" variant="inset" @update:collapsed="(value: boolean) => isSidebarCollapsed = value" class="bg-gray-900 text-gray-200">
    <SidebarHeader>
      <SidebarMenu>
        <SidebarMenuItem>
    ...
    <SidebarContent class="flex-grow overflow-y-auto custom-scrollbar">
      <div v-if="user">
        <div v-for="group in mainNavItems" :key="group.group">
          <SidebarMenuButton @click="toggleGroup(group.group)" class="flex w-full items-center justify-between px-2 py-1 text-sm font-semibold text-gray-400 hover:text-white">
            <span class="flex items-center gap-2 uppercase tracking-wide">
              <component :is="group.icon" class="h-4 w-4 text-blue-400" />
            ...
            <template v-for="item in group.items" :key="item.title">
              <SidebarMenuItem v-if="!item.permission || can(item.permission)">
                <SidebarMenuButton as-child>
                  <Link :href="item.routeName ? route(item.routeName) : '#'" class="flex items-center gap-2 px-2 py-1 text-sm text-gray-300 hover:bg-gray-700 rounded-md">
                    <component :is="item.icon" class="h-4 w-4 text-blue-400" />
