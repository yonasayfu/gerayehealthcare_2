...
const handleNavigation = (event: Event) => {
    ...
    // Ensure the event doesn't reset the expanded state
    if (areAllGroupsExpanded.value) {
        // Make sure we don't lose the expanded state when navigating
        const allGroupNames = mainNavItems.value.map((group: SidebarNavGroup) => group.group);
        openGroups.value = allGroupNames;
    } else {
        // Preserve currently open groups when navigating
...
<template>
    <Sidebar collapsible="icon" v-model:collapsed="isSidebarCollapsed" class="sidebar">
        ...
        <SidebarFooter class="border-t pt-2">
            <SidebarMenu>
                <SidebarMenuItem>
                    <SidebarMenuButton @click.stop="toggleAllGroups" class="w-full justify-between rounded-md px-2 py-2 text-sm hover:bg-muted/30">
