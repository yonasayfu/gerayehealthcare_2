...
import { ref, watch, nextTick, computed } from 'vue';
import { useForm, usePage } from '@inertiajs/vue3';
import { Send, X, MessageSquareText, Search, Minus, GripVertical, Paperclip, Smile } from 'lucide-vue-next';
    ...
    >
      ...
      <div class="p-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-700">
        <h2 class="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-2">
          <MessageSquareText class="w-5 h-5 text-indigo-500" /> Chat messages
        </h2>
        <div class="flex items-center gap-2">
          <button type="button" class="px-3 py-1 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition">
            Let's Chat App
          </button>
          <button @click="emit('minimize')" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 p-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition">
            <Minus class="w-5 h-5" />
          </button>
      ...
      <div class="flex-grow flex min-h-0">
        ...
        <div class="w-1/3 flex flex-col bg-gray-100 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <div class="flex-grow overflow-y-auto custom-scrollbar">
            ...
            >
              <img :src="convo.profile_photo_url || `https://ui-avatars.com/api/?name=${convo.name}&color=7F9CF5&background=EBF4FF`" :alt="convo.name" class="w-10 h-10 rounded-full object-cover">
              <!-- Fallback to initial if no image -->
              <div v-if="!convo.profile_photo_url" class="w-10 h-10 rounded-full bg-indigo-200 dark:bg-indigo-700 flex items-center justify-center text-indigo-800 dark:text-indigo-200 text-sm font-medium">
                {{ convo.name.charAt(0).toUpperCase() }}
              </div>
              <div>
                <p class="font-medium text-gray-900 dark:text-gray-100 truncate">{{ convo.name }}</p>
                <p class="text-xs text-gray-500 dark:text-gray-400 truncate">{{ convo.staff?.position || 'User' }}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <template v-else>
            ...
            <div class="p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700">
              <form @submit.prevent="submit" class="flex items-center gap-3">
                <img :src="authUser.profile_photo_url || 'https://mdbcdn.b-cdn.net/img/Photos/new-templates/bootstrap-chat/ava3-bg.webp'" :alt="authUser.name" class="w-10 h-10 rounded-full mr-2 object-cover">
                <input
                  ...
                  autocomplete="off"
                />
                <a href="#" @click.prevent class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
      ...
      <!-- Resize handle -->
      <div class="resize-handle absolute bottom-0 right-0 w-8 h-8 cursor-se-resize flex items-center justify-center text-gray-400 hover:text-gray-600 dark:text-gray-600 dark:hover:text-gray-400 transition">
...
.resize-handle { background: transparent; border-radius: 3px; }
