...
console.log('GeraChat component script loaded. (Fixed all issues)');
...
const onEmojiSelect = (emoji: { i: string }) => {
  if (messageInput.value) {
    ...
    // Use unified emoji if native isn't available
...
const fetchMessages = async (recipientId: number | null = null, searchQuery: string = '') => {
  ...
  try {
    ...
    let newSelectedConversation: Conversation | null = response.data.selectedConversation || null;
    ...
    messages.value = rawMessages.filter((msg: any) => 
    ...
    messages.value = []; // Ensure messages is always an array
  } finally {
    loading.value = false;
  }
};
...
const submit = async () => {
  ...
  try {
    await form.post(route('messages.store'), {
      ...
      onError: (errors) => {
        alert('Failed to send message: ' + JSON.stringify(errors));
      }
    });
  } catch (error) {
        ...
        <div v-show="!showConversationList || currentWindowWidth >= 768"
          ...
          <template v-else>
            <div class="p-4 border-b border-border bg-card flex justify-between items-center">
              ...
              <button v-if="currentWindowWidth < 768" @click="showConversationList = true" class="text-muted-foreground hover:text-foreground transition-colors px-3 py-1 rounded-md border border-border">
                Back
              </button>
            </div>
