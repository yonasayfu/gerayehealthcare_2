...
const allAdminNavItems: SidebarNavGroup[] = [
    ...
    },
    {
        ...
        items: [
            ...
            {
                title: 'Transactions',
                routeName: 'admin.inventory-transactions.index',
                icon: ClipboardList,
...
// Prevent navigation from affecting group state
const handleNavigation = (event: Event) => {
    // Prevent event propagation to avoid closing groups
    event.stopPropagation();

    // Store current open groups before navigation
    const currentOpenGroups = [...openGroups.value];

    // Use nextTick to ensure this runs after Vue updates the DOM
    nextTick(() => {
        // If all groups should be expanded, make sure they stay that way
        if (areAllGroupsExpanded.value) {
            const allGroupNames = mainNavItems.value.map((group: SidebarNavGroup) => group.group);
            openGroups.value = allGroupNames;
        } else {
            // Otherwise preserve the specific open groups
            openGroups.value = currentOpenGroups;
        }
    });
};
...
<template>
    <Sidebar collapsible="icon" v-model:collapsed="isSidebarCollapsed" class="sidebar">
        <SidebarHeader>
            <SidebarMenu>
                <SidebarMenuItem>
                ...
                <div v-for="group in mainNavItems" :key="group.group" class="mb-1">
                    <div class="group/collapsible">
                        <div
                            ...
                            class="flex w-full cursor-pointer items-center justify-between gap-2 rounded-md px-3 py-2 font-medium text-sidebar-foreground transition-colors duration-150 hover:bg-accent hover:text-accent-foreground"
                            :title="group.group"
                        >
                        ...
                        <div
                            ...
                            @click.stop
                        >
                            <SidebarMenu class="mt-1 space-y-1 pl-4">
                                <template v-for="item in group.items" :key="item.title">
                                    <SidebarMenuItem v-if="!item.permission || can(item.permission)">
                                        <SidebarMenuButton as-child @click.stop="handleNavigation">
                                            <Link
                                                :href="item.routeName ? route(item.routeName) : '#'"
                                                class="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-sm hover:bg-muted/30"
                                                preserve-scroll
                                                preserve-state
            ...
            <SidebarMenu>
                <SidebarMenuItem>
                    <SidebarMenuButton
                        ...
                        class="w-full justify-between rounded-md px-2 py-2 text-sm hover:bg-muted/30"
