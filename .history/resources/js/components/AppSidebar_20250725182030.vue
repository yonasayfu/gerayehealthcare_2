...
// Track only 2 open groups maximum
...
// Toggle group with max 2 open rule
...
// Expand only Patient Management group
const expandPatientGroup = () => {
  const patientGroup = 'Patient Management'
  if (!openGroups.value.includes(patientGroup)) {
    if (openGroups.value.length >= 2) {
      openGroups.value.shift() // Remove oldest if at limit
    }
    openGroups.value.push(patientGroup)
  }
  ...
  >
        ...
        <SidebarMenu>
            <div v-for="group in mainNavItems" :key="group.group" class="mb-1">
                ...
                <div class="group/collapsible">
                    <div 
                        @click.stop="toggleGroup(group.group)"
                                ...
                                <SidebarMenuItem v-if="!item.permission || can(item.permission)">
                                    <SidebarMenuButton as-child>
                                        <Link 
                                            ...
                                            preserve-scroll 
                                            preserve-state
                                            @click.stop="" <!-- Prevent click from bubbling up -->
                                        >
    ...
    <SidebarFooter class="border-t pt-2">
        <SidebarMenu>
            <SidebarMenuItem>
                <SidebarMenuButton 
                    @click="expandPatientGroup"
                    class="w-full justify-between px-2 py-2 text-sm hover:bg-muted/30 rounded-md"
                >
                    <div class="flex items-center gap-2">
                        <Maximize2 class="h-4 w-4 flex-shrink-0" />
                        <span class="truncate">Expand Patient Group</span>
                    </div>
                </SidebarMenuButton>
            </SidebarMenuItem>
        </SidebarMenu>
