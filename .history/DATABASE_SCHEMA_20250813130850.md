...
This document defines the official database schema for the Home-to-Home Care Automation Platform, derived directly from the Laravel migration files. It is organized by functional modules.

---

## Conventions, Constraints, and Indexes

- **Table/Column naming:** snake_case; plural table names. Primary key `id` (SERIAL PRIMARY KEY).
- **Timestamps:** `created_at`, `updated_at` (TIMESTAMP with timezone).
- **Foreign keys:** Declared with explicit `ON DELETE` behavior.
- **Indexes:** Applied for frequent lookups, FK columns, and common filters.
- **Soft deletes:** `deleted_at TIMESTAMP NULL` for soft-deletable models.
